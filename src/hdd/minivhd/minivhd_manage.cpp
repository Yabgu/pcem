...
static bool mvhd_parent_path_exists(struct MVHDPaths *paths, uint32_t plat_code) {
        ...
        if (plat_code == MVHD_DIF_LOC_W2RU && *paths->w2ru_path) {
                cwk_ret = cwk_path_join((const char *)paths->dir_path, (const char *)paths->w2ru_path, paths->joined_path,
                                        sizeof paths->joined_path);
        } else if (plat_code == MVHD_DIF_LOC_W2KU && *paths->w2ku_path) {
                memcpy(paths->joined_path, paths->w2ku_path, (sizeof paths->joined_path) - 1);
                cwk_ret = 0;
        } else if (plat_code == 0) {
                cwk_ret = cwk_path_join((const char *)paths->dir_path, (const char *)paths->file_name, paths->joined_path,
                                        sizeof paths->joined_path);
        }
...
static char *mvhd_get_diff_parent_path(MVHDMeta *vhdm, MVHDError *err) {
        ...
        utf_ret = UTF16BEToUTF8((unsigned char *)paths->file_name, &utf_outlen,
                                (const unsigned char *)vhdm->sparse.par_utf16_name, &utf_inlen);
        if (utf_ret < 0) {
...
MVHDMeta *mvhd_open(const char *path, bool readonly, MVHDError *err) {
        ...
        vhdm->f = readonly ? mvhd_fopen((const char *)vhdm->filename, "rb", err)
                           : mvhd_fopen((const char *)vhdm->filename, "rb+", err);
        if (vhdm->f == NULL) {
