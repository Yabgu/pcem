...
static inline void esdi_clear_irq() { picintc(1 << 14); }
...
static void esdi_write(uint16_t port, uint8_t val, void *p) {
        ...
        //        pclog("esdi_write: port=%04x val=%02x %04x:%04x %02x %i\n", port, val,CS,cpu_state.pc, esdi->status,
        //        esdi->irq_in_progress);
        switch (port) {
        ...
        case 0x3510: /*Command Interface Register*/
                ...
                if (((esdi->cmd_data[0] & CMD_SIZE_4) && esdi->cmd_pos == 4) ||
                    (!(esdi->cmd_data[0] & CMD_SIZE_4) && esdi->cmd_pos == 2)) {
                        ...
                        //                        esdi->cmd_data[1], esdi->cmd_data[2], esdi->cmd_data[3]);
...
static void esdi_mca_write(int port, uint8_t val, void *p) {
        ...
        if (esdi->pos_regs[2] & 1) {
                ...
                if (!(esdi->pos_regs[3] & 8)) {
                        ...
                        //                        pclog("ESDI BIOS ROM at %05x %02x\n", ((esdi->pos_regs[3] & 7) * 0x4000) +
                        //                        0xc0000, esdi->pos_regs[3]);
                }
        }
}
...
static int esdi_available() { return rom_present("90x8969.bin") && rom_present("90x8970.bin"); }

device_t hdd_esdi_device = {
        "IBM ESDI Fixed Disk Adapter (MCA)", DEVICE_MCA, esdi_init, esdi_close, esdi_available, NULL, NULL, NULL, NULL};
