...
void setpitclock(float clock) {
        ...
        //        pclog("CPUMULTI=%g\n", ((14318184.0*(double)(1 << TIMER_SHIFT)) /
        //        (double)models[model]->cpu[cpu_manufacturer].cpus[cpu].rspeed));
...
static void pit_load(PIT *pit, int t) {
        ...
        //        pclog("pit_load: t=%i running=%i thit=%i enabled=%i m=%i l=%x c=%g gate=%i\n", t, pit.running[t], pit.thit[t],
        //        pit.enabled[t], pit.m[t], pit.l[t], pit.c[t], pit.gate[t]);
}
...
static void pit_over(PIT *pit, int t) {
        ...
        //        timer_get_remaining_u64(&pit->timer[t]), pit->thit[t], tsc);
        switch (pit->m[t]) {
        case 0: /*Interrupt on terminal count*/
        case 1: /*Hardware retriggerable one-shot*/
                if (!pit->thit[t])
...
void pit_set_out_func(PIT *pit, int t, void (*func)(int new_out, int old_out)) { pit->set_out_funcs[t] = func; }
