...
static uint32_t get_laserxt_ems_addr(uint32_t addr) {
        if (laserxt_emspage[(addr >> 14) & 3] & 0x80) {
                addr = (romset == ROM_LTXT ? 0x70000 + (((mem_size + 64) & 255) << 10)
                                           : 0x30000 + (((mem_size + 320) & 511) << 10)) +
                       ((laserxt_emspage[(addr >> 14) & 3] & 0x0F) << 14) + ((laserxt_emspage[(addr >> 14) & 3] & 0x40) << 12) +
                       (addr & 0x3FFF);
        }
...
static void laserxt_write(uint16_t port, uint8_t val, void *priv) {
        ...
        case 0xC209:
                ...
                mem_mapping_set_addr(&laserxt_ems_mapping[0], 0xC0000 + (((laserxt_ems_baseaddr_index + 4) & 0x0C) << 14),
                                     0x4000);
                mem_mapping_set_addr(&laserxt_ems_mapping[1], 0xC4000 + (((laserxt_ems_baseaddr_index + 3) & 0x0C) << 14),
                                     0x4000);
                mem_mapping_set_addr(&laserxt_ems_mapping[2], 0xC8000 + (((laserxt_ems_baseaddr_index + 2) & 0x0C) << 14),
                                     0x4000);
...
void laserxt_init() {
        ...
        if (mem_size > 640) {
                ...
                mem_mapping_set_addr(&ram_low_mapping, 0,
                                     romset == ROM_LTXT ? 0x70000 + (((mem_size + 64) & 255) << 10)
                                                        : 0x30000 + (((mem_size + 320) & 511) << 10));
        }
        ...
        for (i = 0; i < 4; i++) {
                ...
                mem_mapping_add(&laserxt_ems_mapping[i], 0xE0000 + (i << 14), 0x4000, mem_read_laserxtems, NULL, NULL,
                                mem_write_laserxtems, NULL, NULL, ram + 0xA0000 + (i << 14), 0, NULL);
