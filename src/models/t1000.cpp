...
static void ems_set_port(struct t1000_system *sys, uint8_t val) {
        ...
        if (sys->ems_port) {
                for (n = 0; n <= 0xC000; n += 0x4000) {
                        io_removehandler(sys->ems_port + n, 0x01, ems_in, NULL, NULL, ems_out, NULL, NULL, sys);
                }
                sys->ems_port = 0;
        }
        val &= 0x0F;
        ...
        if (val == 7) /* No EMS */
        {
                ...
                for (n = 0; n <= 0xC000; n += 0x4000) {
                        io_sethandler(sys->ems_port + n, 0x01, ems_in, NULL, NULL, ems_out, NULL, NULL, sys);
                }
                sys->ems_port = 0;
        }
        /*	pclog(" -> %04x\n", sys->ems_port); */
...
static int addr_to_page(uint32_t addr) { return (addr - 0xD0000) / 0x4000; }
...
static uint8_t read_t1000_ctl(uint16_t addr, void *priv) {
        ...
        case 0x0F:
                ...
                case 0x51:
                        return sys->ems_base | 0x60;
                case 0x52:
                        return sys->is_640k ? 0x80 : 0;
                }
                return 0xFF;
        default:
                return sys->sys_ctl[addr & 0x0F];
        }
}
...
static uint8_t read_t1200_nvram(uint32_t addr, void *priv) { return t1200_nvram[addr & 0x7FF]; }
...
void xt_t1000_init() {
        ...
        mem_mapping_add(&t1000.rom_mapping, 0xA0000, 0x10000, t1000_read_rom, t1000_read_romw, t1000_read_roml, NULL, NULL, NULL,
                        NULL, MEM_MAPPING_INTERNAL, &t1000);
        mem_mapping_disable(&t1000.rom_mapping);

        /* Map the EMS page frame */
        for (pg = 0; pg < 4; pg++) {
                mem_mapping_add(&t1000.mapping[pg], 0xD0000 + (0x4000 * pg), 16384, ems_read_ram, ems_read_ramw, ems_read_raml,
                                ems_write_ram, ems_write_ramw, ems_write_raml, NULL, MEM_MAPPING_EXTERNAL, &t1000);
                /* Start them all off disabled */
                mem_mapping_disable(&t1000.mapping[pg]);
        }

        /* Non-volatile RAM for CONFIG.SYS */
        io_sethandler(0xC0, 4, read_t1000_nvram, NULL, NULL, write_t1000_nvram, NULL, NULL, &t1000);
        /* ROM drive */
        io_sethandler(0xC8, 1, read_t1000_rom_ctl, NULL, NULL, write_t1000_rom_ctl, NULL, NULL, &t1000);
        /* System control functions, and add-on memory board */
        io_sethandler(0xE0, 0x10, read_t1000_ctl, NULL, NULL, write_t1000_ctl, NULL, NULL, &t1000);
...
void xt_t1200_init() {
        ...
        common_init();
        mem_add_bios();
        mem_mapping_add(&t1000.nvr_mapping, 0xF0000, 2048, read_t1200_nvram, NULL, NULL, write_t1200_nvram, NULL, NULL, NULL, 0,
                        &t1000);
        pit_set_out_func(&pit, 1, pit_refresh_timer_xt);
        keyboard_xt_init();
        nmi_init();
