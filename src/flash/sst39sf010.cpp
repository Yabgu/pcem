...
static void set_id_mode(sst_t *sst) {
        ...
        for (c = 0; c < 8; c++) {
                mem_mapping_set_handler(&bios_mapping[c], sst_read_id, NULL, NULL, sst_write, NULL, NULL);
                mem_mapping_set_p(&bios_mapping[c], sst);
                mem_mapping_set_handler(&bios_high_mapping[c], sst_read_id, NULL, NULL, sst_write, NULL, NULL);
...
static void clear_id_mode(sst_t *sst) {
        ...
        for (c = 0; c < 8; c++) {
                mem_mapping_set_handler(&bios_mapping[c], mem_read_bios, mem_read_biosw, mem_read_biosl, sst_write, NULL, NULL);
                mem_mapping_set_p(&bios_mapping[c], sst);
                mem_mapping_set_handler(&bios_high_mapping[c], mem_read_bios, mem_read_biosw, mem_read_biosl, sst_write, NULL,
                                        NULL);
                mem_mapping_set_p(&bios_high_mapping[c], sst);
        }
        sst->id_mode = 0;
...
device_t sst_39sf010_device = {"SST 39SF010 Flash BIOS", 0, sst_39sf010_init, sst_39sf010_close, NULL, NULL, NULL, NULL, NULL};
