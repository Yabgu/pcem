...
static int get_track_nr(uint32_t pos) {
        ...
        for (c = first_track; c < last_track; c++) {
                uint32_t track_address =
                        toc[c].cdte_addr.msf.frame + (toc[c].cdte_addr.msf.second * 75) + (toc[c].cdte_addr.msf.minute * 75 * 60);
...
static int read_toc(int fd, struct cdrom_tocentry *btoc) {
        ...
        for (track = 0; track < 256; track++) {
                ...
                //c, track, btoc[c].cdte_track, btoc[c].cdte_ctrl, btoc[c].cdte_adr, 0, btoc[c].cdte_addr.msf.minute,
                //btoc[c].cdte_addr.msf.second, btoc[c].cdte_addr.msf.frame);
        }
...
static int ioctl_ready(void) {
        ...
        //	pclog("CDROMREADTOCENTRY: addr=%02i:%02i:%02i\n", toc_entry.cdte_addr.msf.minute, toc_entry.cdte_addr.msf.second,
        ...
        if ((toc_entry.cdte_addr.msf.minute != toc[toc_hdr.cdth_trk1].cdte_addr.msf.minute) ||
            ...
            (toc_entry.cdte_addr.msf.frame != toc[toc_hdr.cdth_trk1].cdte_addr.msf.frame) || !tocvalid) {
                ioctl_cd_state = CD_STOPPED;
...
static uint8_t ioctl_getcurrentsubchannel(uint8_t *b, int msf) {
        ...
        uint32_t track_address = toc[track].cdte_addr.msf.frame + (toc[track].cdte_addr.msf.second * 75) +
