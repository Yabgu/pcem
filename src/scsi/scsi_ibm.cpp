...
typedef enum { SCB_STATE_IDLE, SCB_STATE_START, SCB_STATE_WAIT, SCB_STATE_COMPLETE } sbc_state_t;
        ...
        do {
                ...
                case SCB_STATE_START:
                        ...
                        case CMD_SEND_OTHER_SCSI:
                                ...
                                //                                pclog("\n");
                                scsi->cdb.data[1] = (scsi->cdb.data[1] & 0x1f) |
                                                    (scsi->dev_id[scsi->scb_id].lun_id << 5); /*Patch correct LUN into command*/
                                scsi->cdb.idx = 0;
...
static void process_immediate_command(scsi_ibm_t *scsi) {
        ...
        case CMD_ASSIGN:
                //                pclog("Assign: adapter dev=%x scsi ID=%i LUN=%i\n", (scsi->command >> 16) & 0xf, (scsi->command
                //                >> 20) & 7, (scsi->command >> 24) & 7);
...
static void scsi_callback(void *p) {
        ...
        if (scsi->attention_waiting && (scsi->scb_state == SCB_STATE_IDLE || (scsi->attention_pending & 0xf0) == 0xe0)) {
...
static void scsi_ibm_mca_write(int port, uint8_t val, void *p) {
        ...
        io_removehandler((((scsi->pos_regs[2] >> 1) & 7) * 8) + 0x3540, 0x0008, scsi_read, scsi_readw, NULL, scsi_write,
                         scsi_writew, NULL, scsi);
        ...
        if (scsi->pos_regs[2] & 1) {
                ...
                io_sethandler((((scsi->pos_regs[2] >> 1) & 7) * 8) + 0x3540, 0x0008, scsi_read, scsi_readw, NULL, scsi_write,
                              scsi_writew, NULL, scsi);
                if ((scsi->pos_regs[2] & 0xf0) != 0xf0) {
                        ...
                        //                        pclog("SCSI BIOS ROM at %05x %02x\n", ((scsi->pos_regs[2] >> 4) * 0x2000) +
                        //                        0xc0000, scsi->pos_regs[2]);
                }
        }
}
...
static int scsi_ibm_available() { return rom_present("92F2244.U68") && rom_present("92F2245.U69"); }

device_t scsi_ibm_device = {"IBM SCSI Adapter with Cache (MCA)",
                            DEVICE_MCA,
                            scsi_ibm_init,
                            scsi_ibm_close,
                            scsi_ibm_available,
