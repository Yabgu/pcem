...
int primed = 0;
typedef enum { SCSI_AHA1542C, SCSI_BT545S } scsi_type_t;
...
static void clear_irq(aha154x_t *scsi) { picintc(1 << scsi->irq); }
...
static uint8_t aha154x_in(uint16_t port, void *p) {
        ...
        if (CS != 0xc800) {
                ...
                //                cpu_state.pc, ins, scsi->cmd_state);
        }
...
static void process_cdb(aha154x_t *scsi) {
        ...
        case CCB_INITIATOR_RESIDUAL:
                ...
                if (scsi->mb_format == MB_FORMAT_4) {
                        ...
                        scsi->ccb.data_len = mem_readb_phys(scsi->ccb.addr + 0x06) |
                                             (mem_readb_phys(scsi->ccb.addr + 0x05) << 8) |
                                             (mem_readb_phys(scsi->ccb.addr + 0x04) << 16);
                        scsi->ccb.data_pointer = mem_readb_phys(scsi->ccb.addr + 0x09) |
                                                 (mem_readb_phys(scsi->ccb.addr + 0x08) << 8) |
                                                 (mem_readb_phys(scsi->ccb.addr + 0x07) << 16);
                        scsi->ccb.link_pointer = mem_readb_phys(scsi->ccb.addr + 0x0c) |
                        ...
                        //                        scsi->ccb.data_pointer, scsi->ccb.data_len);
                }
                ...
                //                pclog("  scsi_len=%i data_len=%06x data_pointer=%06x\n", scsi->ccb.scsi_cmd_len,
                //                scsi->ccb.data_len, scsi->ccb.data_pointer);
        ...
        case CCB_INITIATOR_SCATTER_GATHER_RESIDUAL:
                ...
                //                mem_readl_phys(scsi->ccb.addr),mem_readl_phys(scsi->ccb.addr+4),mem_readl_phys(scsi->ccb.addr+8),mem_readl_phys(scsi->ccb.addr+12),mem_readl_phys(scsi->ccb.addr+16),
                //                scsi->mb_format == MB_FORMAT_4,  scsi->ccb.opcode);
                temp = mem_readb_phys(scsi->ccb.addr + 0x01);
                //                pclog("Read addr+ctrl %02x %06x\n", temp, scsi->ccb.addr+0x01);
                scsi->ccb.transfer_dir = (temp >> 3) & 3;
                scsi->ccb.scsi_cmd_len = mem_readb_phys(scsi->ccb.addr + 0x02);
                ...
                if (scsi->mb_format == MB_FORMAT_4) {
                        ...
                        scsi->ccb.data_seg_list_len = mem_readb_phys(scsi->ccb.addr + 0x06) |
                                                      (mem_readb_phys(scsi->ccb.addr + 0x05) << 8) |
                                                      (mem_readb_phys(scsi->ccb.addr + 0x04) << 16);
                        scsi->ccb.data_seg_list_pointer = mem_readb_phys(scsi->ccb.addr + 0x09) |
                                                          (mem_readb_phys(scsi->ccb.addr + 0x08) << 8) |
                                                          (mem_readb_phys(scsi->ccb.addr + 0x07) << 16);
                ...
                if (scsi->ccb.residual) {
                        ...
                        if (scsi->mb_format == MB_FORMAT_4) {
                                for (c = 0; c < scsi->ccb.data_seg_list_len; c += 6) {
                                        uint32_t len = mem_readb_phys(addr + c + 2) | (mem_readb_phys(addr + c + 1) << 8) |
                        ...
                        default:
                                if (scsi->type == SCSI_AHA1542C) {
                                        ...
                                        if (scsi->command > 0x22 && (scsi->command < 0xfa || scsi->command > 0xfc) &&
                                            !(scsi->command >= 0x81 && scsi->command <= 0x9d) && scsi->command != 0xda &&
                                            scsi->command != 0xdc)
                                                invalid = 1;
                                        else
                ...
                case COMMAND_START_BIOS_COMMAND:
                        ...
                        if (!scsi->bios_cmd_state)
                                pclog("Start BIOS command %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", scsi->params[0],
                                      scsi->params[1], scsi->params[2], scsi->params[3], scsi->params[4], scsi->params[5],
                                      scsi->params[6], scsi->params[7], scsi->params[8], scsi->params[9]);
                        if (scsi->params[0] >= 0x16) {
                                ...
                                case 0x02: /*Read Sector(s) Into Memory*/
                                        ...
                                        case 0: {
                                                ...
                                                scsi->ccb.data_pointer =
                                                        scsi->params[9] | (scsi->params[8] << 8) | (scsi->params[7] << 16);
                                                ;
                                ...
                                case 0x08: /*Get Drive Parameters*/
                                        ...
                                        case 1:
                                                ...
                                                pclog("Got size %06x %02x%02x%02x%02x\n", addr, scsi->int_buffer[3],
                                                      scsi->int_buffer[2], scsi->int_buffer[1], scsi->int_buffer[0]);
                                                scsi->data_in = 0;
                                                set_irq(scsi, ISR_HACC);
                                                scsi->status |= STATUS_DF;
                                                scsi->cmd_state = CMD_STATE_IDLE;
...
static void process_ccb(aha154x_t *scsi) {
                        ...
                        if (scsi->ccb.from_mailbox) {
                                ...
                                if (scsi->mb_format == MB_FORMAT_4) {
                                        ...
                                        mem_writel_phys(scsi->mba_i + scsi->current_mbi * 8 + 4,
                                                        MBI_CCB_COMPLETE_WITH_ERROR << 24);
                                }
        ...
        case CCB_STATE_SEND_REQUEST_SENSE:
                if (scsi->ccb.req_sense_len == CCB_SENSE_NONE) {
                        ...
                        if (scsi->ccb.from_mailbox) {
                                ...
                                //                                scsi->ccb.total_data_len, scsi->ccb.bytes_transferred,
                                //                                scsi->ccb.residual, scsi->current_mbi);
                                ...
                                if (scsi->mb_format == MB_FORMAT_4) {
                                        ...
                                        if (scsi->current_mbo_is_bios) {
                                                ...
                                                if (scsi->ccb.status == STATUS_GOOD)
                                                        ...
                                                        mem_writeb_phys(scsi->mba_i + scsi->current_mbi * 4,
                                        ...
                                        if (scsi->ccb.status == STATUS_GOOD)
                                                ...
                                                mem_writel_phys(scsi->mba_i + scsi->current_mbi * 8 + 4,
                                                                (MBI_CCB_COMPLETE_WITH_ERROR << 24) | (scsi->ccb.status << 8));
                                }
        ...
        case CCB_STATE_WAIT_REQUEST_SENSE:
                ...
                if (scsi->ccb.from_mailbox) {
                        uint32_t residue = scsi->ccb.total_data_len - scsi->ccb.bytes_transferred;
                        //                        pclog("residue=%x %x %x\n", residue, scsi->ccb.total_data_len,
                        //                        scsi->ccb.bytes_transferred);
                        mem_writeb_phys(scsi->ccb.addr + 0xe, HOST_STATUS_COMMAND_COMPLETE);
                        mem_writeb_phys(scsi->ccb.addr + 0xf, scsi->ccb.status);
...
static void process_scsi(aha154x_t *scsi) {
        ...
        case SCSI_STATE_SEND_COMMAND:
                while (scsi->cdb.idx < scsi->cdb.len && bytes_transferred < MAX_BYTES_TRANSFERRED_PER_POLL) {
                        ...
                        //                        if (!scsi->cdb.idx && scsi->cdb.data[scsi->cdb.idx] == 0x25 && scsi->current_mbo
                        //                        == 1)
                        //                                output = 3;
                        scsi->cdb.idx++;
                        bytes_transferred++;
                       ...
                       bytes_transferred < MAX_BYTES_TRANSFERRED_PER_POLL) {
                        ...
                        for (d = 0; d < 20; d++) {
                                ...
                                if (bus_state & BUS_REQ) {
                                        ...
                                        //                                        scsi->cdb.data_idx, scsi->cdb.data_pointer +
                                        //                                        scsi->cdb.data_idx);
                                        scsi_bus_update(&scsi->bus, bus_out | BUS_ACK);
                                        scsi_bus_update(&scsi->bus, bus_out & ~BUS_ACK);
                                        break;
                                        ...
                                        if (scsi->mb_format == MB_FORMAT_4) {
                                                scsi->cdb.data_len = mem_readb_phys(scsi->cdb.data_seg_list_pointer +
                                                                     ...
                                                                     (mem_readb_phys(scsi->cdb.data_seg_list_pointer +
                                                                      ...
                                                                      << 16);
                                                scsi->cdb.data_pointer = mem_readb_phys(scsi->cdb.data_seg_list_pointer +
                                                ...
                                                scsi->cdb.data_len = mem_readl_phys(scsi->cdb.data_seg_list_pointer +
                                                                                    scsi->cdb.data_seg_list_idx);
                                                scsi->cdb.data_pointer = mem_readl_phys(scsi->cdb.data_seg_list_pointer +
                                                                                        scsi->cdb.data_seg_list_idx + 4);
                                                scsi->cdb.data_idx = 0;
...
static void *scsi_aha1542c_init() {
        ...
        io_sethandler(addr, 0x0004, aha154x_in, NULL, NULL, aha154x_out, NULL, NULL, scsi);
        for (c = 0; c < 8; c++) {
...
static int scsi_aha1542c_available() { return rom_present("adaptec_aha1542c_bios_534201-00.bin"); }

static int scsi_bt545s_available() { return rom_present("BusLogic_BT-545S_U15_27128_5002026-4.50.bin"); }

static device_config_t aha1542c_config[] = {{.name = "addr",
                                             .description = "Address",
                                             .type = CONFIG_SELECTION,
                                             .selection = {{.description = "130", .value = 0x130},
                                                           {.description = "134", .value = 0x134},
                                                           {.description = "230", .value = 0x230},
                                                           {.description = "234", .value = 0x234},
                                                           {.description = "330", .value = 0x330},
                                                           {.description = "334", .value = 0x334},
                                                           {.description = ""}},
                                             .default_int = 0x334},
                                            {.name = "bios_addr",
                                             .description = "BIOS address",
                                             .type = CONFIG_SELECTION,
                                             .selection = {{.description = "C8000", .value = 0xc8000},
                                                           {.description = "CC000", .value = 0xcc000},
                                                           {.description = "D0000", .value = 0xd0000},
                                                           {.description = "D4000", .value = 0xd4000},
                                                           {.description = "D8000", .value = 0xd8000},
                                                           {.description = "DC000", .value = 0xdc000},
                                                           {.description = ""}},
                                             .default_int = 0xdc000},
                                            {.type = -1}};

static device_config_t bt545s_config[] = {{.name = "addr",
                                           .description = "Address",
                                           .type = CONFIG_SELECTION,
                                           .selection = {{.description = "130", .value = 0x130},
                                                         {.description = "134", .value = 0x134},
                                          ...
                                          {.name = "irq",
                                           .description = "IRQ",
                                           .type = CONFIG_SELECTION,
                                           .selection = {{.description = "IRQ 9", .value = 9},
                                          ...
                                          {.name = "dma",
                                           .description = "DMA",
                                           .type = CONFIG_SELECTION,
                                           .selection = {{.description = "DMA 5", .value = 5},
                                          ...
                                          {.name = "host_id",
                                           .description = "Host ID",
                                           .type = CONFIG_SELECTION,
                                           .selection = {{.description = "ID 0", .value = 0},
...
device_t scsi_aha1542c_device = {"Adaptec AHA-1542C (SCSI)",
                                 DEVICE_AT,
                                 scsi_aha1542c_init,
                                 scsi_aha1542c_close,
                                 ...
                                 NULL,
                                 aha1542c_config};
device_t scsi_bt545s_device = {"BusLogic BT-545S (SCSI)",
                               DEVICE_AT,
                               scsi_bt545s_init,
                               scsi_aha1542c_close,
