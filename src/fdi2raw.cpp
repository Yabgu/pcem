...
static uae_u8 *expand_tree(uae_u8 *stream, NODE *node) {
        if (temp & temp2) {
                ...
                node->left = (struct node*)fdi_malloc(sizeof(NODE));
                memset(node->left, 0, sizeof(NODE));
                stream_temp = expand_tree(stream, node->left);
                node->right = (struct node*)fdi_malloc(sizeof(NODE));
...
static void amiga_data(FDI *fdi, uae_u8 *secbuf) {
        ...
        for (i = 0; i < 512; i += 4) {
                deven = ((secbuf[i + 0] << 24) | (secbuf[i + 1] << 16) | (secbuf[i + 2] << 8) | (secbuf[i + 3]));
                dodd = deven >> 1;
                deven &= 0x55555555;
                dodd &= 0x55555555;
...
static void amiga_sector_header(FDI *fdi, uae_u8 *header, uae_u8 *data, int sector, int untilgap) {
        ...
        deven = ((headerbuf[0] << 24) | (headerbuf[1] << 16) | (headerbuf[2] << 8) | (headerbuf[3]));
        dodd = deven >> 1;
        deven &= 0x55555555;
        dodd &= 0x55555555;
        mfmbuf[0] = (uae_u16)(dodd >> 16);
        ...
        mfmbuf[3] = (uae_u16)deven;
        for (i = 0; i < 16; i += 4) {
                deven = ((databuf[i] << 24) | (databuf[i + 1] << 16) | (databuf[i + 2] << 8) | (databuf[i + 3]));
                dodd = deven >> 1;
                deven &= 0x55555555;
                dodd &= 0x55555555;
...
static void s1b(FDI *fdi) {
        int shift = *fdi->track_src++;
        debuglog("s1b:shift=%d,crc=%s,data=%s", shift, datalog(fdi->track_src + (128 << shift), 2),
                 datalog(fdi->track_src, 128 << shift));
...
/* atari st 9 sector */
static void track_atari_st_9(struct fdi *fdi) { track_atari_st(fdi, 9); }
/* atari st 10 sector */
static void track_atari_st_10(struct fdi *fdi) { track_atari_st(fdi, 10); }
/* pc 8 sector */
static void track_pc_8(struct fdi *fdi) { track_pc(fdi, 8); }
/* pc 9 sector */
static void track_pc_9(struct fdi *fdi) { track_pc(fdi, 9); }
/* pc 15 sector */
static void track_pc_15(struct fdi *fdi) { track_pc(fdi, 15); }
/* pc 18 sector */
static void track_pc_18(struct fdi *fdi) { track_pc(fdi, 18); }
/* pc 36 sector */
static void track_pc_36(struct fdi *fdi) { track_pc(fdi, 36); }
...
static uae_u8 *fdi_decompress(int pulses, uae_u8 *sizep, uae_u8 *src, int *dofree) {
        ...
        *dofree = 0;
        ...
        if (mode == 0) {
                ...
                dst = (uint8_t*)fdi_malloc(pulses * 4);
                *dofree = 1;
...
static void fdi2_decode(FDI *fdi, uint32_t totalavg, uae_u32 *avgp, uae_u32 *minp, uae_u32 *maxp, uae_u8 *idx, int maxidx,
                        int *indexoffsetp, int pulses, int mfm) {
        uint32_t adjust;
        uint32_t adjusted_pulse;
        uint32_t standard_MFM_2_bit_cell_size = totalavg / 50000;
        uint32_t standard_MFM_8_bit_cell_size = totalavg / 12500;
        ...
        /* detects a long-enough stable pulse coming just after another stable pulse */
        i = 1;
        while ((i < pulses) && ((idx[i] < maxidx) || (idx[i - 1] < maxidx) ||
                        ...
                        if (idx[i] >= maxidx) { /* stable pulse */
                                ...
                                if ((avg_pulse < min_pulse) || (avg_pulse > max_pulse)) {
                                        ...
                                        outlog("FDI: avgp=%u (%u) minp=%u (%u) maxp=%u (%u) jitter=%d i=%d ni=%d\n", avgp[i],
                                               avgp[nexti], minp[i], minp[nexti], maxp[i], maxp[nexti], jitter, i, nexti);
                                }
...
static int decode_lowlevel_track(FDI *fdi, int track, struct fdi_cache *cache) {
        ...
        if (get_u24(p1 + 9)) {
                ...
                idxp = (uint8_t*)fdi_malloc(pulses * 2);
                idx_free = 1;
                for (i = 0; i < pulses; i++) {
                        idxp[i * 2 + 0] = 2;
                        idxp[i * 2 + 1] = 0;
...
int fdi2raw_get_last_head(FDI *fdi) { return fdi->last_head; }

int fdi2raw_get_rotation(FDI *fdi) { return fdi->rotation_speed; }

int fdi2raw_get_bit_rate(FDI *fdi) { return fdi->bit_rate; }

int fdi2raw_get_type(FDI *fdi) { return fdi->disk_type; }

int fdi2raw_get_write_protect(FDI *fdi) { return fdi->write_protect; }
...
FDI *fdi2raw_header(FILE *f) {
        ...
        if ((fdi->header[140] != 1 && fdi->header[140] != 2) ||
            (fdi->header[141] != 0 && !(fdi->header[140] == 2 && fdi->header[141] == 1))) {
                fdi_free(fdi);
                return NULL;
        }

        fdi->mfmsync_buffer = (int*)fdi_malloc(MAX_MFM_SYNC_BUFFER * sizeof(int));
        fdi->track_src_buffer = (uint8_t*)fdi_malloc(MAX_SRC_BUFFER);
        fdi->track_dst_buffer = (uint8_t*)fdi_malloc(MAX_DST_BUFFER);
        fdi->track_dst_buffer_timing = (uint16_t*)fdi_malloc(MAX_TIMING_BUFFER);
...
int fdi2raw_loadrevolution_2(FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming, int track, int *tracklength, int *indexoffsetp,
        ...
        fdi2_decode(fdi, cache->totalavg, cache->avgp, cache->minp, cache->maxp, cache->idxp, cache->maxidx, &idx, cache->pulses,
                    mfm);
        // fdi2_gcr_decode (fdi, totalavg, avgp, minp, maxp, idxp, idx_off1, idx_off2, idx_off3, maxidx, pulses);
        outlog("track %d: nbits=%d avg len=%.2f weakbits=%d idx=%d\n", track, bitoffset, (double)cache->totalavg / bitoffset,
               cache->weakbits, cache->indexoffset);
        len = fdi->out;
...
int fdi2raw_loadtrack(FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming, int track, int *tracklength, int *indexoffsetp,
        ...
        outlog("track %d: srclen: %d track_type: %02.2X, bitrate: %d\n", fdi->current_track, fdi->track_src_len, fdi->track_type,
               fdi->bit_rate);
