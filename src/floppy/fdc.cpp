...
void fdc_update_is_nsc(int is_nsc) { fdc.is_nsc = is_nsc; }

void fdc_update_enh_mode(int enh_mode) { fdc.enh_mode = enh_mode; }

int fdc_get_rwc(int drive) { return fdc.rwc[drive]; }

void fdc_update_rwc(int drive, int rwc) { fdc.rwc[drive] = rwc; }

int fdc_get_boot_drive() { return fdc.boot_drive; }

void fdc_update_boot_drive(int boot_drive) { fdc.boot_drive = boot_drive; }

void fdc_update_densel_polarity(int densel_polarity) { fdc.densel_polarity = densel_polarity; }

void fdc_update_densel_force(int densel_force) { fdc.densel_force = densel_force; }

void fdc_update_drvrate(int drive, int drvrate) { fdc.drvrate[drive] = drvrate; }
...
int fdc_get_bitcell_period() { return fdc.bitcell_period; }
                ...
                if (fdc.pnum == fdc.ptot) {
                        ...
                        if (fdc.pnum == fdc.ptot) {
                                ...
                                case 0x42: /*Read track*/
                                        ...
                                        //                                        pclog("Read a track track=%i head=%i sector=%i
                                        //                                        eot=%i\n", fdc.track[fdc.drive], fdc.head,
                                        //                                        fdc.sector, fdc.eot[fdc.drive]);
...
uint8_t fdc_read(uint16_t addr, void *priv) {
        ...
        //        /*if (addr!=0x3f4) */printf("Read FDC %04X %04X:%04X %04X %i %02X %02x %i
        //        ",addr,cs>>4,pc,BX,fdc.pos,fdc.st0,fdc.stat,ins);
        switch (addr & 7) {
        case 1: /*???*/
                drive = (fdc.dor & 1) ^ fdd_swap;
                if (!fdc.enable_3f1)
        ...
        case 7: /*Disk change*/
                ...
                if (fdc.dskchg_activelow) /*PC2086/3086 seem to reverse this bit*/
                        temp ^= 0x80;
                //                printf("- DC %i %02X %02X %i %i - ",fdc.dor & 1, fdc.dor, 0x10 << (fdc.dor & 1),
                //                discchanged[fdc.dor & 1], driveempty[fdc.dor & 1]);
                temp |= 1;
...
int fdc_discchange_read() { return (disc_changed[fdc.drive] || drive_empty[fdc.drive]) ? 1 : 0; }

void fdc_set_dskchg_activelow() { fdc.dskchg_activelow = 1; }

void fdc_3f1_enable(int enable) { fdc.enable_3f1 = enable; }

void fdc_set_ps1() { fdc.ps1 = 1; }
