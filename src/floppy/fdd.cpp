...
void fdd_setswap(int swap) { fdd_swap = swap ? 1 : 0; }
...
int fdd_can_read_medium(int drive) {
        ...
        //	pclog("Drive %02X, type %02X, hole flag %02X, flags %02X, result %02X\n", drive, fdd[drive].type, hole,
        //drive_types[fdd[drive].type].flags, drive_types[fdd[drive].type].flags & hole);
