                        ...
                        if ((bufSize != 0) && (scMapCount != 0)) {
                                ...
                                for (j = 0; j < scMapCount * 2; j += 2) {
                                        ...
                                        pclog("Scan code mapping %u detected: %X -> %X\n", scancode_unmapped, scancode_mapped,
                                              scancode_map[scancode_unmapped]);
                                }
...
int window_create() {
        ...
        hwnd = CreateWindowEx(0,                                             /* Extended possibilites for variation */
                              ...
                              480 + (GetSystemMetrics(SM_CYFIXEDFRAME) * 2) + GetSystemMetrics(SM_CYMENUSIZE) +
                                      GetSystemMetrics(SM_CYCAPTION) + 1, /* and height in pixels */
...
void window_close() {
        ...
        if (window) {
                ...
                if (window_remember) {
                        // These are incorrectly set sometimes from SDL_WINDOWEVENT_MOVED so we ensure they have the correct
                        // value.
                        window_x = rect.x;
                        window_y = rect.y;
                }
                        ...
                        if (window_remember) {
                                ...
                                if (!(flags & SDL_WINDOW_FULLSCREEN) && !(flags & SDL_WINDOW_FULLSCREEN_DESKTOP)) {
                                        if (event.window.event == SDL_WINDOWEVENT_MOVED) {
                                                ...
                                                // pclog("render window_remember SDL_WINDOWEVENT_MOVED. setting window_x/y to
                                                // %dx%d\n", window_x, window_y);
