...
int gusfreqs[] = {44100, 41160, 38587, 36317, 34300, 32494, 30870, 29400, 28063, 26843,
                  25725, 24696, 23746, 22866, 22050, 21289, 20580, 19916, 19293};

double vol16bit[4096];

enum { MIDI_INT_RECEIVE = 0x01, MIDI_INT_TRANSMIT = 0x02, MIDI_INT_MASTER = 0x80 };

enum { MIDI_CTRL_TRANSMIT_MASK = 0x60, MIDI_CTRL_TRANSMIT = 0x20, MIDI_CTRL_RECEIVE = 0x80 };

enum { GUS_INT_MIDI_TRANSMIT = 0x01, GUS_INT_MIDI_RECEIVE = 0x02 };

enum { GUS_TIMER_CTRL_AUTO = 0x01 };
...
void gus_update_int_status(gus_t *gus) {
        ...
        //        pclog("gus_update_int_status: tctrl=%02x irqstatus=%02x irq_pending=%i midi_irq_pending=%i gus->irq=%i
        //        gus->irq_midi=%i\n",
...
void writegus(uint16_t addr, uint8_t val, void *p) {
        ...
        case 0x344: /*Global low*/
                ...
                case 0x6: /*Ramp frequency*/
                        ...
                        //                        printf("RFREQ %02X %i %i
                        //                        %f\n",val,gus->voice,gus->rfreq[gus->voice],(double)(val & 63)/(double)(1 <<
                        //                        (3*(val >> 6))));
                        break;
        ...
        case 0x345: /*Global high*/
                ...
                case 0: /*Voice control*/
                        if (!(val & 1) && gus->ctrl[gus->voice] & 1) {
                                //                                printf("Voice on %i - start %05X end %05X freq
                ...
                case 0x6: /*Ramp frequency*/
                        ...
                        //                        pclog("Ramp freq %02X %i %i %f %i\n", val, gus->voice, gus->rfreq[gus->voice],
                        //                        (double)(val & 63)/(double)(1 << (3*(val >> 6))), ins);
                        break;
                case 0x7: /*Ramp start*/
                ...
                case 0xD: /*Ramp control*/
                        ...
                        if (gus->rampirqs[gus->voice] != old)
                                gus_update_int_status(gus);
                        //                        printf("Ramp control %02i %02X %02X
                        ...
                        if (val & 1 && gus->dma != -1) {
                                ...
                                if (val & 2) {
                                        ...
                                        while (c < 65536) {
                                                ...
                                                if (val & 0x04) {
                                                        uint32_t gus_addr =
                                                                (gus->dmaaddr & 0xc0000) | ((gus->dmaaddr & 0x1ffff) << 1);
...
uint8_t readgus(uint16_t addr, void *p) {
        ...
        case 0x343:
                return gus->global;
        case 0x344: /*Global low*/
                    //                /*if (gus->global!=0x43 && gus->global!=0x44) */printf("Reading register %02X
                ...
                break;
        case 0x345: /*Global high*/
                    //                /*if (gus->global!=0x43 && gus->global!=0x44) */printf("HReading register %02X
                    //                %02X\n",gus->global,gus->voice);
                switch (gus->global) {
                case 0x80: /*Voice control*/
                           //                        pclog("Read voice control %02i %02X\n", gus->voice,
                           //                        gus->ctrl[gus->voice]|(gus->waveirqs[gus->voice]?0x80:0));
                ...
                case 0x8D:
                        //                        pclog("Read ramp control %02X %04X %08X  %08X
                        //                        %08X\n",gus->rctrl[gus->voice]|(gus->rampirqs[gus->voice]?0x80:0),gus->rcur[gus->voice]
                        //                        >> 14,gus->rfreq[gus->voice],gus->rstart[gus->voice],gus->rend[gus->voice]);
                ...
                case 0x8F: /*IRQ status*/
                        ...
                        //                        gus->waveirqs[gus->irqstatus2&0x1F], gus->rampirqs[gus->irqstatus2&0x1F]);
                        return val;
        ...
        for (d = 0; d < 32; d++) {
                                ...
                                if (gus->cur[d] <= gus->start[d]) {
                                        ...
                                        if (gus->ctrl[d] & 8) {
                                                if (gus->ctrl[d] & 0x10)
                                                        gus->ctrl[d] ^= 0x40;
                                                gus->cur[d] =
                                                        (gus->ctrl[d] & 0x40) ? (gus->end[d] - diff) : (gus->start[d] + diff);
                ...
                if (!(gus->rctrl[d] & 3)) {
                        if (gus->rctrl[d] & 0x40) {
                                ...
                                if (gus->rcur[d] <= gus->rstart[d]) {
                                        ...
                                        if (!(gus->rctrl[d] & 8)) {
                                                ...
                                                if (gus->rctrl[d] & 0x10)
                                                        gus->rctrl[d] ^= 0x40;
                                                gus->rcur[d] =
                                                        (gus->rctrl[d] & 0x40) ? (gus->rend[d] - diff) : (gus->rstart[d] + diff);
                                        }
                                ...
                                gus->rcur[d] += gus->rfreq[d];
                                //                                        if (d == 1) printf("RCUR+ %i %08X %08X %08X
...
device_t gus_device = {"Gravis UltraSound", 0, gus_init, gus_close, NULL, gus_speed_changed, NULL, gus_add_status_info};
