...
void azt1605_create_config_word(void *p) {
        ...
        uint8_t cd_type = 0; // TODO: see if the cd-rom was originally connected there on the real machines emulated by PCem
                             // (Packard Bell Legend 100CD, Itautec Infoway Multimidia, etc)
        uint8_t cd_dma8 = -1;
        uint8_t cd_irq = 0;
...
void azt1605_config_write(uint16_t addr, uint8_t val, void *p) {
        ...
        if (addr == azt2316a->cur_addr + 0x404) {
                ...
                case 2:
                        ...
                        io_removehandler(azt2316a->cur_wss_addr, 0x0004, azt2316a_wss_read, NULL, NULL, azt2316a_wss_write, NULL,
                                         NULL, azt2316a);
                        io_removehandler(azt2316a->cur_wss_addr + 0x0004, 0x0004, ad1848_read, NULL, NULL, ad1848_write, NULL,
                                         NULL, &azt2316a->ad1848);
                        ...
                        io_sethandler(azt2316a->cur_wss_addr, 0x0004, azt2316a_wss_read, NULL, NULL, azt2316a_wss_write, NULL,
                                      NULL, azt2316a);
                        io_sethandler(azt2316a->cur_wss_addr + 0x0004, 0x0004, ad1848_read, NULL, NULL, ad1848_write, NULL, NULL,
                                      &azt2316a->ad1848);
...
// How it behaves when one or another is activated may affect games auto-detecting (and will also use more of the limited system
// resources!)
...
void *azt_common_init(const int type, char *nvr_filename) {
        ...
        if (type == SB_SUBTYPE_CLONE_AZT2316A_0X11) {
                azt2316a->config_word =
                        read_eeprom[11] + (read_eeprom[12] << 8) + (read_eeprom[13] << 16) + (read_eeprom[14] << 24);
                ...
                // these are not present on the EEPROM
                azt2316a->cur_dma =
                        1; // TODO: investigate TSR to make this work with it - there is no software configurable DMA8?
                azt2316a->cur_wss_irq = 10;
                azt2316a->cur_wss_dma = 0;
                azt2316a->cur_mode = 0;
        } else
        ...
        io_sethandler(azt2316a->cur_addr + 0x0400, 0x0040, azt2316a_config_read, NULL, NULL, azt2316a_config_write, NULL, NULL,
                      azt2316a);
        ...
        io_sethandler(azt2316a->cur_addr + 4, 0x0002, sb_ct1345_mixer_read, NULL, NULL, sb_ct1345_mixer_write, NULL, NULL,
                      azt2316a->sb);
        mpu401_uart_init(&azt2316a->sb->mpu, azt2316a->cur_mpu401_addr, azt2316a->cur_mpu401_irq, 1);

        azt2316a_create_config_word(azt2316a); // recreate even if we have read it from EEPROM, because we have some overrides (CD
                                               // interfaces always off, etc)
        sound_add_handler(azt2316a_get_buffer, azt2316a);

        return azt2316a;
}

void *azt2316a_init() { return azt_common_init(SB_SUBTYPE_CLONE_AZT2316A_0X11, "azt2316a.nvr"); }

void *azt1605_init() { return azt_common_init(SB_SUBTYPE_CLONE_AZT1605_0X0C, "azt1605.nvr"); }
...
void azt_common_close(void *p, char *nvr_filename) {
        ...
        if (f) {
                ...
                // TODO: should remember to save wss duplex setting if PCem has voice recording implemented in the future? Also,
                // default azt2316a->wss_config
...
void azt2316a_close(void *p) { azt_common_close(p, "azt2316a.nvr"); }

void azt1605_close(void *p) { azt_common_close(p, "azt1605.nvr"); }
...
device_t azt2316a_device = {"Aztech Sound Galaxy Pro 16 AB (Washington)",
                            0,
                            azt2316a_init,
                            azt2316a_close,
                            NULL,
