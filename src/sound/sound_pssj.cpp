...
static void pssj_write(uint16_t port, uint8_t val, void *p) {
        ...
        case 0:
                ...
                if (!pssj->enable && ((val & 4) && (pssj->ctrl & 3)))
                        timer_set_delay_u64(&pssj->timer,
                                            (TIMER_USEC * (1000000.0 / 3579545.0) * (double)(pssj->freq ? pssj->freq : 0x400)));
...
device_t pssj_device = {"Tandy PSSJ", 0, pssj_init, pssj_close, NULL, NULL, NULL, NULL};
