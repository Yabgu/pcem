...
enum { PAS16_PCM_MONO = 0x20, PAS16_PCM_ENA = 0x40 };

enum { PAS16_SC2_16BIT = 0x04, PAS16_SC2_MSBINV = 0x10 };

enum { PAS16_FILT_MUTE = 0x20 };
...
static void pas16_pit_out(uint16_t port, uint8_t val, void *p) {
        ...
        case 3: /*CTRL*/
                if ((val & 0xC0) == 0xC0) {
                        if (!(val & 0x20)) {
                                if (val & 2)
                                        pas16->pit.rl[0] = timer_get_remaining_u64(&pit.timer[0]) / PITCONST;
                                ;
                                if (val & 4)
...
static uint8_t pas16_readdma(pas16_t *pas16) { return dma_channel_read(pas16->dma); }
...
device_t pas16_device = {"Pro Audio Spectrum 16", DEVICE_NOT_WORKING, pas16_init, pas16_close, NULL, NULL, NULL, NULL};
