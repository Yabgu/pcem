                                        ...
                                        if (status >= 0 && device != -1) {
                                                ...
                                                for (sub = 0; sub < sub_nr; sub++) {
                                                        ...
                                                        if (snd_ctl_rawmidi_info(ctl, info) == 0) {
                                                                ...
                                                                snprintf(midi_devices[midi_device_count].name, 50,
        ...
        if (midi_len) {
                if (midi_pos < 3) {
                        ...
                        if (midi_pos == midi_len) {
                                //				pclog("MIDI send %i: %02x %02x %02x %02x\n", midi_len, midi_command[0],
                                //midi_command[1], midi_command[2], midi_command[3]);
...
void midi_get_dev_name(int num, char *s) { strcpy(s, midi_devices[num].name); }
