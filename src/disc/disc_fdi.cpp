...
void fdi_seek(int drive, int track) {
        ...
        for (density = 0; density < 4; density++) {
                int c = fdi2raw_loadtrack(fdi[drive].h, (uint16_t *)fdi[drive].track_data[0][density], (uint16_t *)fdi_timing,
                                          track * fdi[drive].sides, &fdi[drive].tracklen[0][density],
                ...
                if (fdi[drive].sides == 2) {
                        c = fdi2raw_loadtrack(fdi[drive].h, (uint16_t *)fdi[drive].track_data[1][density], (uint16_t *)fdi_timing,
                                              (track * fdi[drive].sides) + 1, &fdi[drive].tracklen[1][density],
...
void fdi_writeback(int drive, int track) { return; }
...
static void calccrc(uint8_t byte) { crc = (crc << 8) ^ CRCTable[(crc >> 8) ^ byte]; }
...
void fdi_poll() {
        ...
        for (bitcount = 0; bitcount < 16; bitcount++) {
                ...
                tempi = fdi[fdi_drive].track_data[fdi_side][fdi_density][((fdi_pos >> 3) & 0xFFFF) ^ 1] &
                        (1 << (7 - (fdi_pos & 7)));
                fdi_pos++;
                fdi_buffer <<= 1;
                        ...
                        if (!pollbitsleft) {
                                ...
                                if (readidpoll) {
                                        ...
                                        if (!pollbytesleft) {
                                                ...
                                                //                                        fdi_sectordat[0], fdi_sectordat[2],
                                                //                                        fdi_track, fdi_sector);
                                                if ((fdi_sectordat[0] == fdi_track && (fdi_sectordat[3] == fdi_n) &&
                                                     (fdi_sectordat[2] == fdi_sector || fdi_sector == SECTOR_NEXT)) ||
                                                    fdi_inreadaddr) {
                                                        ...
                                                        if ((crc >> 8) != fdi_sectordat[4] || (crc & 0xFF) != fdi_sectordat[5]) {
                                                                ...
                                                                inreadop = 0;
                                                                if (fdi_inreadaddr)
                                                                        fdc_sectorid(fdi_sectordat[0], fdi_sectordat[1],
                                                                                     fdi_sectordat[2], fdi_sectordat[3],
                                                                                     fdi_sectordat[4], fdi_sectordat[5]);
                                                                else
                                                                        fdc_headercrcerror();
                                                                return;
                                                        }
                                                        ...
                                                        if (fdi_inreadaddr) {
                                                                fdc_sectorid(fdi_sectordat[0], fdi_sectordat[1], fdi_sectordat[2],
                                                                             fdi_sectordat[3], fdi_sectordat[4],
                                                                             fdi_sectordat[5]);
                                                                fdi_inreadaddr = 0;
                                                        }
                                                }
                                ...
                                if (readdatapoll) {
                                        ...
                                        if (!pollbytesleft) {
                                                fdi_inread = 0;
                                                //#if 0
                                                if ((crc >> 8) != sectorcrc[0] ||
                                                    (crc & 0xFF) !=
                                                            sectorcrc[1]) // || (fditrack==79 && fdisect==4 && fdc_side&1))
                                                {
                                                        //                                                pclog("Data CRC error :
                                                        //                                                %02X %02X %02X %02X %i
                                                        //                                                %04X
                                                ...
                                                //#endif
                                                //                                        pclog("End of FDI read %02X %02X %02X
