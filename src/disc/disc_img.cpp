...
void img_load(int drive, const char *fn) {
        ...
        if (bpb_disable || (bpb_sides < 1) || (bpb_sides > 2) || (bpb_bps < 128) || (bpb_bps > 2048)) {
                ...
                if (size <= (160 * 1024)) {
                  ...
                  // else if (size == 1884160)      { img[drive].sectors = 23; img[drive].tracks = 80; bit_rate_300 = 500; } /*XDF
                // format - used by OS/2 Warp*/ else if (size == 1763328)      { img[drive].sectors = 21; img[drive].tracks = 82;
                // bit_rate_300 = 500; } /*XDF format - used by OS/2 Warp*/
        ...
        if (img[drive].xdf_type) {
                ...
                if (!track) {
                        ...
                        for (sector = 0; sector < sectors_fat; sector++) {
                                disc_sector_add(drive, 0, track, 0, sector + 0x81, 2, img[drive].bitcell_period_300rpm,
                                                &img[drive].track_data[0][current_pos]);
                                current_pos += 512;
                        }
                        disc_sector_add(drive, 1, track, 1, 0x81, 2, img[drive].bitcell_period_300rpm,
                                        &img[drive].track_data[0][current_pos]);
                        current_pos += 512;
                        for (sector = 0; sector < 8; sector++) {
                                disc_sector_add(drive, 0, track, 0, sector + 1, 2, img[drive].bitcell_period_300rpm,
                                                &img[drive].track_data[0][current_pos]);
                                current_pos += 512;
                        }
                        ...
                        for (sector = 0; sector < 14; sector++) {
                                disc_sector_add(drive, 1, track, 1, sector + 0x82, 2, img[drive].bitcell_period_300rpm,
                        ...
                        for (sector = 0; sector < xdf_spt[current_xdft]; sector++) {
                                disc_sector_add(drive, xdf_map[current_xdft][sector][0], track, xdf_map[current_xdft][sector][0],
                                                ...
                                                img[drive].bitcell_period_300rpm, &img[drive].track_data[0][current_pos]);
                        ...
                        for (sector = xdf_spt[current_xdft]; sector < (xdf_spt[current_xdft] << 1); sector++) {
                                disc_sector_add(drive, xdf_map[current_xdft][sector][0], track, xdf_map[current_xdft][sector][0],
                                                xdf_map[current_xdft][sector][2] + 0x80, xdf_map[current_xdft][sector][2],
                                                img[drive].bitcell_period_300rpm, &img[drive].track_data[1][current_pos]);
                                current_pos += (128 << xdf_map[current_xdft][sector][2]);
                        }
                }
        } else {
