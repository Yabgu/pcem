...
/* Default palette */
static unsigned char defpal[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
void incolor_out(uint16_t addr, uint8_t val, void *p) {
        ...
        case 0x3b7:
                ...
                if (incolor->crtc[10] == 6 &&
                    incolor->crtc[11] == 7) /*Fix for Generic Turbo XT BIOS, which sets up cursor registers wrong*/
                {
                        incolor->crtc[10] = 0xb;
                        incolor->crtc[11] = 0xc;
                }
...
void incolor_poll(void *p) {
        ...
        if (!incolor->linepos) {
                ...
                if (incolor->sc == (incolor->crtc[11] & 31) ||
                    ((incolor->crtc[8] & 3) == 3 && incolor->sc == ((incolor->crtc[11] & 31) >> 1))) {
                        incolor->con = 0;
                        incolor->coff = 1;
                }
                if (incolor->vadj) {
                ...
                } else if (incolor->sc == incolor->crtc[9] ||
                           ((incolor->crtc[8] & 3) == 3 && incolor->sc == (incolor->crtc[9] >> 1))) {
                        ...
                        if (oldvc == incolor->crtc[4]) {
                                ...
                                if (!incolor->vadj)
                                        incolor->ma = incolor->maback = (incolor->crtc[13] | (incolor->crtc[12] << 8)) & 0x3fff;
                                if ((incolor->crtc[10] & 0x60) == 0x20)
                                        incolor->cursoron = 0;
                                else
                                        incolor->cursoron = incolor->blink & 16;
                        }
                        if (incolor->vc == incolor->crtc[7]) {
                                ...
                                if (incolor->crtc[7]) {
                                        ...
                                        video_blit_memtoscreen(0, incolor->firstline, 0, incolor->lastline - incolor->firstline,
                                                               xsize, incolor->lastline - incolor->firstline);
                ...
                if ((incolor->sc == (incolor->crtc[10] & 31) ||
                     ((incolor->crtc[8] & 3) == 3 && incolor->sc == ((incolor->crtc[10] & 31) >> 1)))) {
                        incolor->con = 1;
...
device_t incolor_device = {"Hercules InColor", 0, incolor_init, incolor_close, NULL, incolor_speed_changed, NULL, NULL};
