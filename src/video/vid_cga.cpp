...
void cga_poll(void *p) {
        ...
        if (!cga->linepos) {
                ...
                if (cga->cgadispon) {
                        ...
                        if (cga->cgamode & 1) {
                                        ...
                                        if (drawcursor) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[cga->displine])[(x << 3) + c + 8] =
                                                                cols[(fontdat[chr + cga->fontbase][cga->sc & 7] & (1 << (c ^ 7)))
                                                                             ? 1
                                                                             : 0] ^
                                                                0xffffff;
                                        } else {
                                ...
                                for (x = 0; x < cga->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                ((uint32_t *)buffer32->line[cga->displine])[(x << 4) + (c << 1) + 8] =
                                                        ((uint32_t *)buffer32->line[cga->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                cols[dat >> 14];
                                                dat <<= 2;
                                        }
                                }
                ...
                if (cga->composite) {
                        ...
                        for (c = 0; c < x; c++)
                                ((uint32_t *)buffer32->line[cga->displine])[c] =
                                        cgapal[((uint32_t *)buffer32->line[cga->displine])[c] & 0xf];
                }
...
void *cga_standalone_init() {
        ...
        timer_add(&cga->timer, cga_poll, cga, 1);
        mem_mapping_add(&cga->mapping, 0xb8000, 0x08000, cga_read, NULL, NULL, cga_write, NULL, NULL, NULL, MEM_MAPPING_EXTERNAL,
                        cga);
...
device_config_t cga_config[] = {
        ...
        {.name = "snow_enabled", .description = "Snow emulation", .type = CONFIG_BINARY, .default_int = 1},
        {.name = "contrast", .description = "Alternate monochrome contrast", .type = CONFIG_BINARY, .default_int = 0},
        {.type = -1}};

device_t cga_device = {"CGA", 0, cga_standalone_init, cga_close, NULL, cga_speed_changed, NULL, NULL, cga_config};
