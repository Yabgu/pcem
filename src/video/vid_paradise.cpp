...
void paradise_remap(paradise_t *paradise) {
        ...
        if (svga->seqregs[0x11] & 0x80) {
                ...
                paradise->read_bank[1] = paradise->read_bank[3] =
                        ((svga->gdcreg[0x9] & 0x7f) << 12) + ((svga->gdcreg[6] & 0x08) ? 0 : 0x8000);
                paradise->write_bank[0] = paradise->write_bank[2] = (svga->gdcreg[0xa] & 0x7f) << 12;
                paradise->write_bank[1] = paradise->write_bank[3] =
                        ((svga->gdcreg[0xa] & 0x7f) << 12) + ((svga->gdcreg[6] & 0x08) ? 0 : 0x8000);
                ...
                if (svga->gdcreg[0x6] & 0xc) {
                        ...
                        paradise->read_bank[1] = paradise->write_bank[1] =
                                ((svga->gdcreg[0xa] & 0x7f) << 12) + ((svga->gdcreg[6] & 0x08) ? 0 : 0x8000);
                        paradise->read_bank[2] = paradise->write_bank[2] = (svga->gdcreg[0x9] & 0x7f) << 12;
                        paradise->read_bank[3] = paradise->write_bank[3] =
                                ((svga->gdcreg[0x9] & 0x7f) << 12) + ((svga->gdcreg[6] & 0x08) ? 0 : 0x8000);
                }
        } else {
                //              pclog("Remap 4\n");
        ...
        //        pclog("Remap - %04X %04X  %04x %04x\n", paradise->read_bank[0], paradise->write_bank[0], paradise->read_bank[1],
        //        paradise->write_bank[1]);
}
...
void *paradise_pvga1a_init() {
        ...
        mem_mapping_set_handler(&paradise->svga.mapping, paradise_read, paradise_readw, NULL, paradise_write, paradise_writew,
                                NULL);
...
static int oli_go481_available() { return (rom_present("oli_go481_lo.bin") && rom_present("oli_go481_hi.bin")); }

device_t paradise_pvga1a_pc2086_device = {"Paradise PVGA1A (Amstrad PC2086)",
                                          0,
...
device_t paradise_pvga1a_pc3086_device = {"Paradise PVGA1A (Amstrad PC3086)",
                                          0,
                                          ...
                                          paradise_force_redraw,
                                          paradise_add_status_info};
device_t paradise_wd90c11_megapc_device = {"Paradise WD90C11 (Amstrad MegaPC)",
                                           0,
                                           ...
                                           paradise_force_redraw,
                                           paradise_add_status_info};
device_t paradise_pvga1a_oli_go481_device = {"Paradise PVGA1A (Olivetti GO481)",
                                             0,
