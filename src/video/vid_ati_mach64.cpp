...
enum { SRC_BG = 0, SRC_FG = 1, SRC_HOST = 2, SRC_BLITSRC = 3, SRC_PAT = 4 };

enum { MONO_SRC_1 = 0, MONO_SRC_PAT = 1, MONO_SRC_HOST = 2, MONO_SRC_BLITSRC = 3 };

enum { BPP_1 = 0, BPP_4 = 1, BPP_8 = 2, BPP_15 = 3, BPP_16 = 4, BPP_32 = 5 };

enum { OP_RECT, OP_LINE };

enum { SRC_PATT_EN = 1, SRC_PATT_ROT_EN = 2, SRC_LINEAR_EN = 4 };

enum { DP_BYTE_PIX_ORDER = (1 << 24) };
...
void mach64_recalctimings(svga_t *svga) {
        ...
        if (((mach64->crtc_gen_cntl >> 24) & 3) == 3) {
                ...
                //                pclog("mach64_recalctimings : frame %i,%i disp %i,%i vsync at %i rowoffset %i pixel clock %f MA
                //                %08X\n", svga->htotal, svga->vtotal, svga->hdisp, svga->dispend, svga->vsyncstart,
                //                svga->rowoffset, svga->clock, svga->ma);
        } else {
...
void mach64_updatemapping(mach64_t *mach64) {
        ...
        case 0x0: /*128k at A0000*/
                mem_mapping_set_handler(&mach64->svga.mapping, mach64_read, mach64_readw, mach64_readl, mach64_write,
                                        mach64_writew, mach64_writel);
        ...
        case 0x8: /*32k at B0000*/
                mem_mapping_set_handler(&mach64->svga.mapping, svga_read, svga_readw, svga_readl, svga_write, svga_writew,
                                        svga_writel);
        ...
        if (mach64->linear_base) {
                if (mach64->type == MACH64_GX) {
                        if ((mach64->config_cntl & 3) == 2) {
                                ...
                                mem_mapping_set_addr(&mach64->mmio_linear_mapping, mach64->linear_base + ((8 << 20) - 0x4000),
                                                     0x4000);
                        } else {
                                /*4 MB aperture*/
                                mem_mapping_set_addr(&mach64->linear_mapping, mach64->linear_base, (4 << 20) - 0x4000);
                                mem_mapping_set_addr(&mach64->mmio_linear_mapping, mach64->linear_base + ((4 << 20) - 0x4000),
                                                     0x4000);
                        }
                } else {
                        /*2*8 MB aperture*/
...
static void mach64_accel_write_fifo(mach64_t *mach64, uint32_t addr, uint8_t val) {
        ...
        case 0x123:
                ...
                if ((addr & 0x3ff) == 0x123 && !(val & 0x80)) {
                        ...
                        if ((mach64->dst_bres_lnth & 0x7fff) && ((mach64->dp_src & 7) != SRC_HOST) &&
                            (((mach64->dp_src >> 8) & 7) != SRC_HOST) && (((mach64->dp_src >> 16) & 3) != MONO_SRC_HOST))
                                mach64_blit(0, -1, mach64);
                }
                break;
        case 0x124:
...
static void mach64_accel_write_fifo_l(mach64_t *mach64, uint32_t addr, uint32_t val) {
        ...
        case 0x23c:
                if (mach64->accel.source_host || (mach64->dp_pix_width & DP_BYTE_PIX_ORDER))
                        ...
                        mach64_blit(((val & 0xff000000) >> 24) | ((val & 0x00ff0000) >> 8) | ((val & 0x0000ff00) << 8) |
                                            ((val & 0x000000ff) << 24),
                                    32, mach64);
                break;
...
void mach64_cursor_dump(mach64_t *mach64) {
        /*        pclog("Mach64 cursor :\n");
                pclog("Ena = %i X = %i Y = %i Addr = %05X Xoff = %i Yoff = %i\n", svga->hwcursor.ena, svga->hwcursor.x,
           svga->hwcursor.y, svga->hwcursor.addr, svga->hwcursor.xoff, svga->hwcursor.yoff);*/
}
...
void mach64_start_fill(mach64_t *mach64) {
...
#ifdef MACH64_DEBUG
        pclog("src %i %i  %i %i  %08X %08X\n", mach64->accel.src_x_count, mach64->accel.src_y_count, mach64->accel.src_width1,
              mach64->accel.src_height1, mach64->src_height1_width1, mach64->src_height2_width2);
#endif
        ...
        mach64->accel.busy = 1;
#ifdef MACH64_DEBUG
        pclog("mach64_start_fill : dst %i, %i  src %i, %i  size %i, %i  src pitch %i offset %X  dst pitch %i offset %X  scissor "
              "%i %i %i %i  src_fg %i  mix %02X %02X\n",
              mach64->accel.dst_x_start, mach64->accel.dst_y_start, mach64->accel.src_x_start, mach64->accel.src_y_start,
              mach64->accel.dst_width, mach64->accel.dst_height, mach64->accel.src_pitch, mach64->accel.src_offset,
              mach64->accel.dst_pitch, mach64->accel.dst_offset, mach64->accel.sc_left, mach64->accel.sc_right,
              mach64->accel.sc_top, mach64->accel.sc_bottom, mach64->accel.source_fg, mach64->accel.mix_fg, mach64->accel.mix_bg);
#endif
                ...
                while (count) {
                        ...
                        case MONO_SRC_BLITSRC:
                                if (mach64->src_cntl & SRC_LINEAR_EN) {
                                        ...
                                        READ(mach64->accel.src_offset + (src_y * mach64->accel.src_pitch) + src_x, mix,
                                             WIDTH_1BIT);
                                }
                                break;
                        }

                        if (dst_x >= mach64->accel.sc_left && dst_x <= mach64->accel.sc_right && dst_y >= mach64->accel.sc_top &&
                            dst_y <= mach64->accel.sc_bottom) {
                                switch (mix ? mach64->accel.source_fg : mach64->accel.source_bg) {
                                case SRC_HOST:
                                        src_dat = host_dat;
                                        break;
                                case SRC_BLITSRC:
                                        READ(mach64->accel.src_offset + (src_y * mach64->accel.src_pitch) + src_x, src_dat,
                                             mach64->accel.src_size);
                                        break;
                                case SRC_FG:
                                ...
                                if (mach64->dst_cntl & DST_POLYGON_EN) {
                                        int poly_src;
                                        READ(mach64->accel.src_offset + (src_y * mach64->accel.src_pitch) + src_x, poly_src,
                                             mach64->accel.src_size);
                                        if (poly_src)
                                                mach64->accel.poly_draw = !mach64->accel.poly_draw;
                                }
                                if (!(mach64->dst_cntl & DST_POLYGON_EN) || mach64->accel.poly_draw) {
                                        READ(mach64->accel.dst_offset + (dst_y * mach64->accel.dst_pitch) + dst_x, dest_dat,
                                        ...
                                        case 4: /*DST_CLR != CLR_CMP_CLR*/
                                                cmp_clr = (((mach64->accel.clr_cmp_src) ? src_dat : dest_dat) &
                                                           mach64->accel.clr_cmp_mask) != mach64->accel.clr_cmp_clr;
                                                break;
                                        ...
                                        if (!cmp_clr)
                                                MIX
                                                        ...
                                                        WRITE(mach64->accel.dst_offset + (dst_y * mach64->accel.dst_pitch) +
                                                                      dst_x,
                                                              mach64->accel.dst_size);
                                }
                        }
                        ...
                        if (mach64->dst_cntl & DST_24_ROT_EN) {
                                mach64->accel.dp_frgd_clr =
                                        ((mach64->accel.dp_frgd_clr >> 8) & 0xffff) | (mach64->accel.dp_frgd_clr << 16);
                                mach64->accel.dp_bkgd_clr =
                                        ((mach64->accel.dp_bkgd_clr >> 8) & 0xffff) | (mach64->accel.dp_bkgd_clr << 16);
                        }
                        ...
                        if (!(mach64->src_cntl & SRC_LINEAR_EN)) {
                                ...
                                if (mach64->accel.src_x_count <= 0) {
                                        mach64->accel.src_x = 0;
                                        if ((mach64->src_cntl & (SRC_PATT_ROT_EN | SRC_PATT_EN)) ==
                        ...
                        if (mach64->accel.x_count <= 0) {
                                ...
                                if (mach64->accel.dst_height <= 0) {
                                        ...
                                        if (mach64->dst_cntl & DST_X_TILE)
                                                mach64->dst_y_x =
                                                        (mach64->dst_y_x & 0xfff) |
                                                        ((mach64->dst_y_x + (mach64->accel.dst_width << 16)) & 0xfff0000);
                                        if (mach64->dst_cntl & DST_Y_TILE)
                                                mach64->dst_y_x =
                                                        (mach64->dst_y_x & 0xfff0000) |
                                                        ((mach64->dst_y_x + (mach64->dst_height_width & 0x1fff)) & 0xfff);
                                        return;
                                }
                ...
                while (count) {
                        ...
                        if (mach64->dst_cntl & DST_POLYGON_EN) {
                                if (mach64->dst_cntl & DST_Y_MAJOR)
                                        draw_pixel = 1;
                                else if ((mach64->dst_cntl & DST_X_DIR) &&
                                         mach64->accel.err < (mach64->dst_bres_dec + mach64->dst_bres_inc)) /*X+*/
                                        draw_pixel = 1;
                        ...
                        if (mach64->accel.dst_x >= mach64->accel.sc_left && mach64->accel.dst_x <= mach64->accel.sc_right &&
                            ...
                            draw_pixel) {
                                ...
                                case SRC_HOST:
                                        ...
                                        break;
                                case SRC_BLITSRC:
                                        READ(mach64->accel.src_offset + (mach64->accel.src_y * mach64->accel.src_pitch) +
                                                     mach64->accel.src_x,
                                             src_dat, mach64->accel.src_size);
                                        break;
                                case SRC_FG:
                                        src_dat = mach64->accel.dp_frgd_clr;
                                        break;
                                ...
                                READ(mach64->accel.dst_offset + (mach64->accel.dst_y * mach64->accel.dst_pitch) +
                                             mach64->accel.dst_x,
                                     dest_dat, mach64->accel.dst_size);

                                //                                pclog("Blit %i,%i %i,%i  %X %X  %i  %02X %02X %i ",
                                //                                mach64->accel.src_x, mach64->accel.src_y, mach64->accel.dst_x,
                                //                                mach64->accel.dst_y,  (mach64->accel.src_offset +
                                //                                (mach64->accel.src_y * mach64->accel.src_pitch) +
                                //                                mach64->accel.src_x) & 0x7fffff, (mach64->accel.dst_offset +
                                //                                (mach64->accel.dst_y * mach64->accel.dst_pitch) +
                                //                                mach64->accel.dst_x) & 0x7fffff, count, src_dat, dest_dat, mix);
...
#ifdef MACH64_DEBUG
                        pclog("x %i y %i err %i inc %i dec %i\n", mach64->accel.dst_x, mach64->accel.dst_y, mach64->accel.err,
                              mach64->dst_bres_inc, mach64->dst_bres_dec);
#endif
        ...
        case 2: /*Data*/
                ...
                for (c = 0; c < 4; c++) {
                        ...
                        pclog("PLLfreq %i = %g  %g m=%02x n=%02x p=%02x\n", c, (2.0 * r * n) / (m * p), p,
                              mach64->pll_regs[PLL_REF_DIV], mach64->pll_regs[VCLK0_FB_DIV + c], mach64->pll_regs[VCLK_POST_DIV]);
...
uint8_t mach64_ext_readb(uint32_t addr, void *p) {
        ...
        if (!(addr & 0x400)) {
                ...
                case 0xc3:
                        if (mach64->type == MACH64_GX)
                                ret = ati68860_ramdac_in((addr & 3) | ((mach64->dac_cntl & 3) << 2), &mach64->ramdac,
                                                         &mach64->svga);
                        else
...
#ifdef MACH64_DEBUG
        if ((addr & 0x3fc) != 0x018)
                pclog("mach64_ext_readb : addr %08X ret %02X\n", addr, ret);
#endif
        return ret;
}
...
#ifdef MACH64_DEBUG
        if ((addr & 0x3fc) != 0x018)
                pclog("mach64_ext_readw : addr %08X ret %04X\n", addr, ret);
#endif
        return ret;
}
...
#ifdef MACH64_DEBUG
        if ((addr & 0x3fc) != 0x018)
                pclog("mach64_ext_readl : addr %08X ret %08X\n", addr, ret);
#endif
        return ret;
}
...
void mach64_ext_writeb(uint32_t addr, uint8_t val, void *p) {
        ...
        if (!(addr & 0x400)) {
                ...
                case 0x63:
                        ...
                        if (mach64->type == MACH64_VT2)
                                mach64->ramdac.pallook[0] =
                                        makecol32((mach64->cur_clr0 >> 24) & 0xff, (mach64->cur_clr0 >> 16) & 0xff,
                                                  (mach64->cur_clr0 >> 8) & 0xff);
                        break;
                case 0x64:
                case 0x65:
                case 0x66:
                case 0x67:
                        ...
                        if (mach64->type == MACH64_VT2)
                                mach64->ramdac.pallook[1] =
                                        makecol32((mach64->cur_clr1 >> 24) & 0xff, (mach64->cur_clr1 >> 16) & 0xff,
                                                  (mach64->cur_clr1 >> 8) & 0xff);
                        break;
                case 0x68:
                case 0x69:
                case 0x6a:
                ...
                case 0xc3:
                        if (mach64->type == MACH64_GX)
                                ati68860_ramdac_out((addr & 3) | ((mach64->dac_cntl & 3) << 2), val, &mach64->ramdac,
                                                    &mach64->svga);
                        else
                ...
                case 0xd3:
                        ...
                        //                if (val == 2) output = 3;
                        ati_eeprom_write(&mach64->eeprom, mach64->gen_test_cntl & 0x10, mach64->gen_test_cntl & 2,
                                         mach64->gen_test_cntl & 1);
...
void mach64_hwcursor_draw(svga_t *svga, int displine) {
        ...
        for (x = 0; x < 64 - svga->hwcursor_latch.xoff; x += 4) {
                ...
                if (!(dat & 2))
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 32] = (dat & 1) ? col1 : col0;
                else if ((dat & 3) == 3)
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 32] ^= 0xFFFFFF;
                dat >>= 2;
                if (!(dat & 2))
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 33] = (dat & 1) ? col1 : col0;
                else if ((dat & 3) == 3)
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 33] ^= 0xFFFFFF;
                dat >>= 2;
                if (!(dat & 2))
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 34] = (dat & 1) ? col1 : col0;
                else if ((dat & 3) == 3)
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 34] ^= 0xFFFFFF;
                dat >>= 2;
                if (!(dat & 2))
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 35] = (dat & 1) ? col1 : col0;
                else if ((dat & 3) == 3)
                        ((uint32_t *)buffer32->line[displine])[svga->hwcursor_latch.x + x + 35] ^= 0xFFFFFF;
                dat >>= 2;
                offset += 4;
        }
        ...
        if (overlay_cmp_mix == 2) {
                ...
                for (x = 0; x < mach64->svga.overlay_latch.xsize; x++) {
                        ...
                        case 1:
                                ...
                                break;
                        case 4:
                                vid_cmp = ((mach64->overlay_dat[h] ^ mach64->overlay_video_key_clr) &
                        ...
                        case 1:
                                ...
                                break;
                        case 4:
                                gr_cmp = (((p[x]) ^ mach64->overlay_graphics_key_clr) & mach64->overlay_graphics_key_msk &
                                          0xffffff);
                                break;
                        case 5:
                                gr_cmp = !(((p[x]) ^ mach64->overlay_graphics_key_clr) & mach64->overlay_graphics_key_msk &
...
static void mach64_io_remove(mach64_t *mach64) {
        ...
        for (c = 0; c < 8; c++) {
                io_removehandler((c * 0x1000) + 0x2ec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl, mach64_ext_outb,
                                 mach64_ext_outw, mach64_ext_outl, mach64);
                io_removehandler((c * 0x1000) + 0x6ec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl, mach64_ext_outb,
                                 mach64_ext_outw, mach64_ext_outl, mach64);
                io_removehandler((c * 0x1000) + 0xaec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl, mach64_ext_outb,
                                 mach64_ext_outw, mach64_ext_outl, mach64);
                io_removehandler((c * 0x1000) + 0xeec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl, mach64_ext_outb,
                                 mach64_ext_outw, mach64_ext_outl, mach64);
        }
        ...
        if (mach64->block_decoded_io && mach64->block_decoded_io < 0x10000)
                io_removehandler(mach64->block_decoded_io, 0x0400, mach64_block_inb, mach64_block_inw, mach64_block_inl,
                                 mach64_block_outb, mach64_block_outw, mach64_block_outl, mach64);
}
...
static void mach64_io_set(mach64_t *mach64) {
        ...
        if (!mach64->use_block_decoded_io) {
                for (c = 0; c < 8; c++) {
                        io_sethandler((c * 0x1000) + 0x2ec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl,
                                      mach64_ext_outb, mach64_ext_outw, mach64_ext_outl, mach64);
                        io_sethandler((c * 0x1000) + 0x6ec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl,
                                      mach64_ext_outb, mach64_ext_outw, mach64_ext_outl, mach64);
                        io_sethandler((c * 0x1000) + 0xaec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl,
                                      mach64_ext_outb, mach64_ext_outw, mach64_ext_outl, mach64);
                        io_sethandler((c * 0x1000) + 0xeec, 0x0004, mach64_ext_inb, mach64_ext_inw, mach64_ext_inl,
                                      mach64_ext_outb, mach64_ext_outw, mach64_ext_outl, mach64);
                }
        }
        ...
        if (mach64->use_block_decoded_io && mach64->block_decoded_io && mach64->block_decoded_io < 0x10000)
                io_sethandler(mach64->block_decoded_io, 0x0400, mach64_block_inb, mach64_block_inw, mach64_block_inl,
                              mach64_block_outb, mach64_block_outw, mach64_block_outl, mach64);
}
...
uint8_t mach64_pci_read(int func, int addr, void *p) {
        ...
        case 0x40:
                return mach64->use_block_decoded_io;
        }
        return 0;
}
...
static void *mach64_common_init() {
        ...
        svga_init(&mach64->svga, mach64, mach64->vram_size << 20, mach64_recalctimings, mach64_in, mach64_out,
                  mach64_hwcursor_draw, mach64_overlay_draw);

        if (PCI)
                mem_mapping_disable(&mach64->bios_rom.mapping);

        mem_mapping_add(&mach64->linear_mapping, 0, 0, svga_read_linear, svga_readw_linear, svga_readl_linear, svga_write_linear,
                        svga_writew_linear, svga_writel_linear, NULL, 0, &mach64->svga);
        mem_mapping_add(&mach64->mmio_linear_mapping, 0, 0, mach64_ext_readb, mach64_ext_readw, mach64_ext_readl,
                        mach64_ext_writeb, mach64_ext_writew, mach64_ext_writel, NULL, 0, mach64);
        mem_mapping_add(&mach64->mmio_linear_mapping_2, 0, 0, mach64_ext_readb, mach64_ext_readw, mach64_ext_readl,
                        mach64_ext_writeb, mach64_ext_writew, mach64_ext_writel, NULL, 0, mach64);
        mem_mapping_add(&mach64->mmio_mapping, 0xbc000, 0x04000, mach64_ext_readb, mach64_ext_readw, mach64_ext_readl,
                        mach64_ext_writeb, mach64_ext_writew, mach64_ext_writel, NULL, 0, mach64);
        mem_mapping_disable(&mach64->mmio_mapping);
...
int mach64gx_available() { return rom_present("mach64gx/bios.bin"); }
int mach64vt2_available() { return rom_present("atimach64vt2pci.bin"); }
...
void mach64_add_status_info(char *s, int max_len, void *p) {
        ...
        sprintf(temps, "%f%% CPU\n%f%% CPU (real)\n\n", ((double)mach64->blitter_time * 100.0) / timer_freq,
                ((double)mach64->blitter_time * 100.0) / status_diff);
        strncat(s, temps, max_len);

        mach64->blitter_time = 0;
}

static device_config_t mach64gx_config[] = {{.name = "memory",
...
device_t mach64gx_device = {"ATI Mach64GX",      0,
                            mach64gx_init,       mach64_close,
                            mach64gx_available,  mach64_speed_changed,
                            mach64_force_redraw, mach64_add_status_info,
                            mach64gx_config};
device_t mach64vt2_device = {"ATI Mach64VT2",     DEVICE_PCI,           mach64vt2_init,      mach64_close,
                             mach64vt2_available, mach64_speed_changed, mach64_force_redraw, mach64_add_status_info,
                             mach64vt2_config};
