        ...
        case 0x3CF:
                ...
                case 0xF:
                        if (val & 1)
                                ...
                                svga->read_bank = (svga->seqregs[0xe] & 0xf) * 65536;
                        svga->write_bank = (svga->seqregs[0xe] & 0xf) * 65536;
                        break;
                }
                break;
        ...
        case 0x3D5:
                ...
                case 0x47:
                        if (tgui->type >= TGUI_9440) {
                                ...
                                svga->hwcursor.addr =
                                        (svga->crtc[0x44] << 10) | ((svga->crtc[0x45] & 0x7) << 18) | (svga->hwcursor.yoff * 8);
                        }
                        break;
...
void tgui_recalctimings(svga_t *svga) {
        ...
        if (tgui->type >= TGUI_9440) {
                if (svga->miscout & 8)
                        svga->clock = (cpuclock * (double)(1ull << 32)) /
                                      (((tgui->clock_n + 8) * 14318180.0) / ((tgui->clock_m + 2) * (1 << tgui->clock_k)));
...
void tgui_recalcmapping(tgui_t *tgui) {
        ...
        if (tgui->type == TGUI_9400CXI) {
                if (tgui->ext_gdc_regs[0] & EXT_CTRL_LATCH_COPY) {
                        mem_mapping_set_handler(&tgui->linear_mapping, tgui_ext_linear_read, NULL, NULL, tgui_ext_linear_write,
                                                tgui_ext_linear_writew, tgui_ext_linear_writel);
                        mem_mapping_set_handler(&svga->mapping, tgui_ext_read, NULL, NULL, tgui_ext_write, tgui_ext_writew,
                                                tgui_ext_writel);
                } else if (tgui->ext_gdc_regs[0] & EXT_CTRL_MONO_EXPANSION) {
                        mem_mapping_set_handler(&tgui->linear_mapping, svga_read_linear, svga_readw_linear, svga_readl_linear,
                                                tgui_ext_linear_write, tgui_ext_linear_writew, tgui_ext_linear_writel);
                        mem_mapping_set_handler(&svga->mapping, svga_read, svga_readw, svga_readl, tgui_ext_write,
                                                tgui_ext_writew, tgui_ext_writel);
                } else {
                        mem_mapping_set_handler(&tgui->linear_mapping, svga_read_linear, svga_readw_linear, svga_readl_linear,
                                                svga_write_linear, svga_writew_linear, svga_writel_linear);
                        mem_mapping_set_handler(&svga->mapping, svga_read, svga_readw, svga_readl, svga_write, svga_writew,
                                                svga_writel);
                }
        }
...
void tgui_hwcursor_draw(svga_t *svga, int displine) {
        ...
        dat[0] = (svga->vram[remapped_addr] << 24) | (svga->vram[remapped_addr + 1] << 16) |
                 (svga->vram[remapped_addr + 2] << 8) | svga->vram[remapped_addr + 3];
        remapped_addr = dword_remap(svga->hwcursor_latch.addr + 4);
        dat[1] = (svga->vram[remapped_addr] << 24) | (svga->vram[remapped_addr + 1] << 16) |
                 (svga->vram[remapped_addr + 2] << 8) | svga->vram[remapped_addr + 3];
        for (xx = 0; xx < 32; xx++) {
...
uint8_t tgui_pci_read(int func, int addr, void *p) {
        ...
        case 0x02:
                return 0x40; /*TGUI9440 (9682)*/
        case 0x03:
                return 0x94;

        case 0x04:
                return 0x03; /*Respond to IO and memory accesses*/
...
static void *tgui_init(const char *bios_fn, int type, int mem_size) {
        ...
        svga_init(&tgui->svga, tgui, tgui->vram_size, tgui_recalctimings, tgui_in, tgui_out, tgui_hwcursor_draw, NULL);

        mem_mapping_add(&tgui->linear_mapping, 0, 0, svga_read_linear, svga_readw_linear, svga_readl_linear,
                        tgui_accel_write_fb_b, tgui_accel_write_fb_w, tgui_accel_write_fb_l, NULL, 0, &tgui->svga);
        mem_mapping_add(&tgui->accel_mapping, 0xbc000, 0x4000, tgui_accel_read, tgui_accel_read_w, tgui_accel_read_l,
                        tgui_accel_write, tgui_accel_write_w, tgui_accel_write_l, NULL, 0, tgui);
        mem_mapping_disable(&tgui->accel_mapping);
...
static void *tgui9400cxi_init() { return tgui_init("9400CXI.vbi", TGUI_9400CXI, 0); }
...
static void *tgui9440_init() { return tgui_init("9440.vbi", TGUI_9440, 0); }

static int tgui9400cxi_available() { return rom_present("9400CXI.vbi"); }

static int tgui9440_available() { return rom_present("9440.vbi"); }
...
static void tgui_ext_linear_writel(uint32_t addr, uint32_t val, void *p) { tgui_ext_linear_writew(addr, val, p); }
...
#define READ(addr, dat)                                                                                                          \
        if (tgui->accel.bpp == 0)                                                                                                \
                dat = svga->vram[dword_remap(addr) & 0x1fffff];                                                                  \
        else                                                                                                                     \
                dat = vram_w[dword_remap_w(addr) & 0xfffff];
...
void tgui_accel_command(int count, uint32_t cpu_dat, tgui_t *tgui) {
        ...
        if (tgui->accel.flags & TGUI_SOLIDFILL) {
                ...
                for (y = 0; y < 8; y++) {
                        for (x = 0; x < 8; x++) {
                                tgui->accel.tgui_pattern[y][x] =
                                        (tgui->accel.pattern[y] & (1 << x)) ? tgui->accel.fg_col : tgui->accel.bg_col;
                        }
                }
        } else {
                if (tgui->accel.bpp == 0) {
                        ...
                        for (y = 0; y < 8; y++) {
                                for (x = 0; x < 8; x++) {
                                        tgui->accel.tgui_pattern[y][x] = tgui->accel.pattern[x * 2 + y * 16] |
                                                                         (tgui->accel.pattern[x * 2 + y * 16 + 1] << 8);
                                }
                        }
                }
        }
        /*	for (y = 0; y < 8; y++)
           ...
           tgui->accel.tgui_pattern[y][0], tgui->accel.tgui_pattern[y][1], tgui->accel.tgui_pattern[y][2],
           tgui->accel.tgui_pattern[y][3], tgui->accel.tgui_pattern[y][4], tgui->accel.tgui_pattern[y][5],
           tgui->accel.tgui_pattern[y][6], tgui->accel.tgui_pattern[y][7]);
                }*/
        ...
        case TGUI_BITBLT:
                ...
                //tgui->accel.src_y, tgui->accel.dst_x, tgui->accel.dst_y, tgui->accel.size_x, tgui->accel.size_y,
                //tgui->accel.flags);
                if (count == -1) {
                ...
                case TGUI_SRCCPU:
                        ...
                        while (count) {
                                ...
                                //				pclog("  %i,%i  %02X %02X %02X  %02X\n", tgui->accel.x, tgui->accel.y,
                                //src_dat,dst_dat,pat_dat, out);
                ...
                case TGUI_SRCMONO | TGUI_SRCCPU:
                        ...
                        while (count) {
                                ...
                                //tgui->accel.y, src_dat,dst_dat,pat_dat, out, (!(tgui->accel.flags & TGUI_TRANSENA) || src_dat !=
                                //trans_col));
                                cpu_dat <<= 1;
                                tgui->accel.src += xdir;
                                tgui->accel.dst += xdir;
                ...
                default:
                        while (count) {
                                ...
                                //                                pclog("  %i,%i  %02X %02X %02X  %02X\n", tgui->accel.x,
                                //                                tgui->accel.y, src_dat,dst_dat,pat_dat, out);
...
static void tgui_accel_write_fifo_fb_b(tgui_t *tgui, uint32_t addr, uint8_t val) { tgui_accel_command(8, val << 24, tgui); }
...
void tgui_add_status_info(char *s, int max_len, void *p) {
        ...
        sprintf(temps, "%f%% CPU\n%f%% CPU (real)\n\n", ((double)tgui->blitter_time * 100.0) / timer_freq,
                ((double)tgui->blitter_time * 100.0) / status_diff);
...
device_t tgui9400cxi_device = {"Trident TGUI 9400CXi", 0,
                               tgui9400cxi_init,       tgui_close,
                               tgui9400cxi_available,  tgui_speed_changed,
                               tgui_force_redraw,      tgui_add_status_info,
                               tgui9440_config};

device_t tgui9400cxi_elx_device = {"Trident TGUI 9400CXi (Elonex PC-425X)",
                                   0,
...
device_t tgui9440_device = {"Trident TGUI 9440", 0,
                            tgui9440_init,       tgui_close,
                            tgui9440_available,  tgui_speed_changed,
                            tgui_force_redraw,   tgui_add_status_info,
                            tgui9440_config};
