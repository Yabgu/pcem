...
uint8_t ega_in(uint16_t addr, void *p) {
        ...
        case 0x3c2:
                ...
                case 0xc:
                        return (egaswitches & 1) ? 0x10 : 0;
                case 0x8:
                        return (egaswitches & 2) ? 0x10 : 0;
                case 0x4:
                        return (egaswitches & 4) ? 0x10 : 0;
                case 0x0:
                        return (egaswitches & 8) ? 0x10 : 0;
                }
                break;
        case 0x3c4:
                return ega->seqaddr;
...
void ega_recalctimings(ega_t *ega) {
        ...
        printf("Recalc! %i %i %i %i   %i %02X\n", ega->vtotal, ega->dispend, ega->vsyncstart, ega->split, ega->hdisp,
               ega->attrregs[0x16]);
        ...
        pclog("dispontime %i (%f)  dispofftime %i (%f)\n", ega->dispontime, (float)ega->dispontime, ega->dispofftime,
              (float)ega->dispofftime);
        //        printf("EGA horiz total %i display end %i clock rate %i vidclock %i
        //        %i\n",crtc[0],crtc[1],egaswitchread,vidclock,((ega3c2>>2)&3) | ((tridentnewctrl2<<2)&4)); printf("EGA vert total
        //        %i display end %i max row %i vsync %i\n",ega_vtotal,ega_dispend,(crtc[9]&31)+1,ega_vsyncstart); printf("total %f
        //        on %f cycles off %f cycles frame %f sec %f
                        ...
                        if (ega->seqregs[1] & 1) {
                                for (xx = 0; xx < 8; xx++)
                                        ((uint32_t *)buffer32->line[ega->displine])[((x << 4) + 32 + (xx << 1)) & 2047] =
                                                ((uint32_t *)buffer32->line[ega->displine])[((x << 4) + 33 + (xx << 1)) & 2047] =
                                                        (dat & (0x80 >> xx)) ? fg : bg;
                        } else {
                                for (xx = 0; xx < 8; xx++)
                                        ((uint32_t *)buffer32->line[ega->displine])[((x * 18) + 32 + (xx << 1)) & 2047] =
                                                ((uint32_t *)buffer32->line[ega->displine])[((x * 18) + 33 + (xx << 1)) & 2047] =
                                                        (dat & (0x80 >> xx)) ? fg : bg;
                                if ((chr & ~0x1f) != 0xc0 || !(ega->attrregs[0x10] & 4))
                                        ...
                                        ((uint32_t *)buffer32->line[ega->displine])[((x * 18) + 32 + 16) & 2047] =
                                                ((uint32_t *)buffer32->line[ega->displine])[((x * 18) + 32 + 17) & 2047] =
                                                        (dat & 1) ? fg : bg;
                        }
                } else {
                        if (ega->seqregs[1] & 1) {
                                for (xx = 0; xx < 8; xx++)
                                        ((uint32_t *)buffer32->line[ega->displine])[((x << 3) + 32 + xx) & 2047] =
                                                (dat & (0x80 >> xx)) ? fg : bg;
                        } else {
                                for (xx = 0; xx < 8; xx++)
                                        ((uint32_t *)buffer32->line[ega->displine])[((x * 9) + 32 + xx) & 2047] =
                                                (dat & (0x80 >> xx)) ? fg : bg;
                                if ((chr & ~0x1f) != 0xc0 || !(ega->attrregs[0x10] & 4))
                                        ((uint32_t *)buffer32->line[ega->displine])[((x * 9) + 32 + 8) & 2047] = bg;
                                else
                                        ((uint32_t *)buffer32->line[ega->displine])[((x * 9) + 32 + 8) & 2047] =
                                                (dat & 1) ? fg : bg;
                        }
                }
                ega->ma += 4;
...
static void ega_draw_2bpp(ega_t *ega) {
        ...
        for (x = 0; x <= ega->hdisp; x++) {
                ...
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 14 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 15 + offset] =
                                ega->pallook[ega->egapal[edat[1] & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 12 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 13 + offset] =
                                ega->pallook[ega->egapal[(edat[1] >> 2) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 10 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 11 + offset] =
                                ega->pallook[ega->egapal[(edat[1] >> 4) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 8 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 9 + offset] =
                                ega->pallook[ega->egapal[(edat[1] >> 6) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 6 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 7 + offset] =
                                ega->pallook[ega->egapal[(edat[0] >> 0) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 4 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 5 + offset] =
                                ega->pallook[ega->egapal[(edat[0] >> 2) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 2 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 3 + offset] =
                                ega->pallook[ega->egapal[(edat[0] >> 4) & 3]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 1 + offset] =
                                ega->pallook[ega->egapal[(edat[0] >> 6) & 3]];
        }
}
...
static void ega_draw_4bpp_lowres(ega_t *ega) {
        ...
        for (x = 0; x <= ega->hdisp; x++) {
                ...
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 14 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 15 + offset] =
                                ega->pallook[ega->egapal[(dat & 0xf) & ega->attrregs[0x12]]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 12 + offset] =
                        ((uint32_t *)buffer32->line[ega->displine])[(x << 4) + 13 + offset] =
                                ega->pallook[ega->egapal[(dat >> 4) & ega->attrregs[0x12]]];
...
static void ega_draw_4bpp_highres(ega_t *ega) {
        ...
        for (x = 0; x <= ega->hdisp; x++) {
                ...
                dat = edatlookup[edat[0] & 3][edat[1] & 3] | (edatlookup[edat[2] & 3][edat[3] & 3] << 2);
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 7 + offset] =
                        ega->pallook[ega->egapal[(dat & 0xf) & ega->attrregs[0x12]]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 6 + offset] =
                        ega->pallook[ega->egapal[(dat >> 4) & ega->attrregs[0x12]]];
                dat = edatlookup[(edat[0] >> 2) & 3][(edat[1] >> 2) & 3] |
                      (edatlookup[(edat[2] >> 2) & 3][(edat[3] >> 2) & 3] << 2);
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 5 + offset] =
                        ega->pallook[ega->egapal[(dat & 0xf) & ega->attrregs[0x12]]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 4 + offset] =
                        ega->pallook[ega->egapal[(dat >> 4) & ega->attrregs[0x12]]];
                dat = edatlookup[(edat[0] >> 4) & 3][(edat[1] >> 4) & 3] |
                      (edatlookup[(edat[2] >> 4) & 3][(edat[3] >> 4) & 3] << 2);
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 3 + offset] =
                        ega->pallook[ega->egapal[(dat & 0xf) & ega->attrregs[0x12]]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 2 + offset] =
                        ega->pallook[ega->egapal[(dat >> 4) & ega->attrregs[0x12]]];
                dat = edatlookup[edat[0] >> 6][edat[1] >> 6] | (edatlookup[edat[2] >> 6][edat[3] >> 6] << 2);
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + 1 + offset] =
                        ega->pallook[ega->egapal[(dat & 0xf) & ega->attrregs[0x12]]];
                ((uint32_t *)buffer32->line[ega->displine])[(x << 3) + offset] =
                        ega->pallook[ega->egapal[(dat >> 4) & ega->attrregs[0x12]]];
        }
}
...
void ega_poll(void *p) {
        ...
        if (!ega->linepos) {
                ...
                if (ega->dispon) {
                        ...
                        if (ega->scrblank) {
                                for (x = 0; x < ega->hdisp; x++) {
                                        ...
                                        case 0:
                                                for (xx = 0; xx < 9; xx++)
                                                        ((uint32_t *)buffer32->line[ega->displine])[(x * 9) + xx + 32] = 0;
                                                break;
                                        case 1:
                                                for (xx = 0; xx < 8; xx++)
                                                        ((uint32_t *)buffer32->line[ega->displine])[(x * 8) + xx + 32] = 0;
                                                break;
                                        case 8:
                                                for (xx = 0; xx < 18; xx++)
                                                        ((uint32_t *)buffer32->line[ega->displine])[(x * 18) + xx + 32] = 0;
                                                break;
                                        case 9:
                                                for (xx = 0; xx < 16; xx++)
                                                        ((uint32_t *)buffer32->line[ega->displine])[(x * 16) + xx + 32] = 0;
                                                break;
                                        }
                                }
                ...
                if (ega->vc == ega->vsyncstart) {
                        ...
                        ega->stat |= 8;
                        if (ega->seqregs[1] & 8)
                                x = ega->hdisp * ((ega->seqregs[1] & 1) ? 8 : 9) * 2;
                        else
                                x = ega->hdisp * ((ega->seqregs[1] & 1) ? 8 : 9);
...
void *ega_standalone_init() {
        ...
        mem_mapping_add(&ega->mapping, 0xa0000, 0x20000, ega_read, NULL, NULL, ega_write, NULL, NULL, NULL, MEM_MAPPING_EXTERNAL,
                        ega);
...
static int ega_standalone_available() { return rom_present("ibm_6277356_ega_card_u44_27128.bin"); }
...
void ega_add_status_info(char *s, int max_len, void *p) {
        ...
        if (!ega->video_bpp)
                strcpy(temps, "EGA in text mode\n");
        else
                sprintf(temps, "EGA colour depth : %i bpp\n", ega->video_bpp);
...
static device_config_t ega_config[] = {
        {.name = "memory",
         ...
         .selection = {{.description = "64 kB", .value = 64},
        ...
        {.name = "monitor_type",
         ...
         .type = CONFIG_SELECTION,
         .selection = {{.description = "EGA Colour, 40x25", .value = 6},
                       {.description = "EGA Colour, 80x25", .value = 7},
                       {.description = "EGA Colour, ECD", .value = 9},
                       {.description = "EGA Monochrome (white)", .value = 10 | (DISPLAY_WHITE << 4)},
                       {.description = "EGA Monochrome (green)", .value = 10 | (DISPLAY_GREEN << 4)},
                       {.description = "EGA Monochrome (amber)", .value = 10 | (DISPLAY_AMBER << 4)},
                       {.description = ""}},
         .default_int = 9},
        {.type = -1}};

device_t ega_device = {
        "EGA",     0, ega_standalone_init, ega_close, ega_standalone_available, ega_speed_changed, NULL, ega_add_status_info,
        ega_config};
