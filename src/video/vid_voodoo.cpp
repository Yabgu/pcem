...
void voodoo_recalc(voodoo_t *voodoo) {
        ...
        /*        pclog("voodoo_recalc : front_offset %08X  back_offset %08X  aux_offset %08X draw_offset %08x\n",
           voodoo->params.front_offset, voodoo->back_offset, voodoo->params.aux_offset, voodoo->params.draw_offset); pclog("
           fb_read_offset %08X  fb_write_offset %08X  row_width %i  %08x %08x\n", voodoo->fb_read_offset, voodoo->fb_write_offset,
           voodoo->row_width, voodoo->lfbMode, voodoo->params.fbzMode);*/
}
        ...
        } else if (addr & 0x400000) /*Framebuffer*/
        {
                ...
                case SST_status: {
                        ...
                        if (SLI_ENABLED && voodoo->type != VOODOO_2) {
                                voodoo_t *voodoo_other =
                                        (voodoo == voodoo->set->voodoos[0]) ? voodoo->set->voodoos[1] : voodoo->set->voodoos[0];
                ...
                case SST_hvRetrace: {
                        ...
                        uint32_t diff = (timer_get_ts_int(&voodoo->timer) > (tsc & 0xffffffff))
                                                ? (timer_get_ts_int(&voodoo->timer) - (tsc & 0xffffffff))
                                                : 0;
...
static void voodoo_writel(uint32_t addr, uint32_t val, void *p) {
        ...
        } else if (addr & 0x400000) /*Framebuffer*/
        {
                ...
                //                pclog("Write CMDFIFO %08x(%08x) %08x  %08x\n", addr, voodoo->cmdfifo_base + (addr & 0x3fffc),
                //                val, (voodoo->cmdfifo_base + (addr & 0x3fffc)) & voodoo->fb_mask);
                *(uint32_t *)&voodoo->fb_mem[(voodoo->cmdfifo_base + (addr & 0x3fffc)) & voodoo->fb_mask] = val;
                ...
                case SST_fbiInit0:
                        if (voodoo->initEnable & 0x01) {
                                ...
                                if (voodoo->set->nr_cards == 2)
                                        svga_set_override(
                                                voodoo->svga,
                                                (voodoo->set->voodoos[0]->fbiInit0 | voodoo->set->voodoos[1]->fbiInit0) & 1);
                                else
                ...
                case SST_fbiInit1:
                        if (voodoo->initEnable & 0x01) {
                                ...
                                //                        voodoo->write_time, voodoo->burst_time);
                        }
                        break;
                case SST_fbiInit2:
                        if (voodoo->initEnable & 0x01) {
                        ...
                        if (val & 0x800) {
                                ...
                                if (voodoo->dac_reg == 5) {
                                        if (!voodoo->dac_reg_ff)
                                                voodoo->dac_pll_regs[voodoo->dac_data[4] & 0xf] =
                                                        (voodoo->dac_pll_regs[voodoo->dac_data[4] & 0xf] & 0xff00) | val;
                                        else
                                                voodoo->dac_pll_regs[voodoo->dac_data[4] & 0xf] =
                                                        (voodoo->dac_pll_regs[voodoo->dac_data[4] & 0xf] & 0xff) | (val << 8);
                                        //                                pclog("Write PLL reg %x %04x\n", voodoo->dac_data[4] &
                                        //                                0xf, voodoo->dac_pll_regs[voodoo->dac_data[4] & 0xf]);
                ...
                case SST_scrFilter:
                        if (voodoo->initEnable & 0x01) {
                                ...
                                voodoo->scrfilterThreshold =
                                        val; /* update the threshold values and generate a new lookup table if necessary */
...
static void voodoo_recalcmapping(voodoo_set_t *set) {
        if (set->nr_cards == 2) {
                if (set->voodoos[0]->pci_enable && set->voodoos[0]->memBaseAddr) {
                        ...
                        } else if (set->voodoos[1]->pci_enable &&
                                   (set->voodoos[0]->memBaseAddr == set->voodoos[1]->memBaseAddr)) {
...
static void voodoo_add_status_info(char *s, int max_len, void *p) {
        ...
        if (voodoo_set->nr_cards == 2) {
                sprintf(temps2, "%f%% CPU (%f%% real)\n", ((double)voodoo_slave->render_time[0] * 100.0) / timer_freq,
                        ((double)voodoo_slave->render_time[0] * 100.0) / status_diff);
                strncat(temps, temps2, sizeof(temps) - 1);

                if (voodoo_slave->render_threads >= 2) {
                        sprintf(temps2, "%f%% CPU (%f%% real)\n", ((double)voodoo_slave->render_time[1] * 100.0) / timer_freq,
                                ((double)voodoo_slave->render_time[1] * 100.0) / status_diff);
                        strncat(temps, temps2, sizeof(temps) - 1);
                }
                if (voodoo_slave->render_threads == 4) {
                        sprintf(temps2, "%f%% CPU (%f%% real)\n%f%% CPU (%f%% real)\n",
                                ((double)voodoo_slave->render_time[2] * 100.0) / timer_freq,
                                ((double)voodoo_slave->render_time[2] * 100.0) / status_diff,
                                ((double)voodoo_slave->render_time[3] * 100.0) / timer_freq,
                                ((double)voodoo_slave->render_time[3] * 100.0) / status_diff);
                        strncat(temps, temps2, sizeof(temps) - 1);
                }
        }
...
static void voodoo_speed_changed(void *p) {
        ...
        voodoo_set->voodoos[0]->write_time =
                pci_nonburst_time + pci_burst_time * ((voodoo_set->voodoos[0]->fbiInit1 & 2) ? 1 : 0);
        ...
        if (voodoo_set->nr_cards == 2) {
                ...
                voodoo_set->voodoos[1]->read_time =
                        pci_nonburst_time + pci_burst_time * ((voodoo_set->voodoos[1]->fbiInit4 & 1) ? 2 : 1);
                voodoo_set->voodoos[1]->write_time =
                        pci_nonburst_time + pci_burst_time * ((voodoo_set->voodoos[1]->fbiInit1 & 2) ? 1 : 0);
                voodoo_set->voodoos[1]->burst_time = pci_burst_time * ((voodoo_set->voodoos[1]->fbiInit1 & 2) ? 2 : 1);
        }
        //        pclog("Voodoo read_time=%i write_time=%i burst_time=%i %08x %08x\n", voodoo->read_time, voodoo->write_time,
        //        voodoo->burst_time, voodoo->fbiInit1, voodoo->fbiInit4);
}
...
void *voodoo_card_init() {
        ...
        mem_mapping_add(&voodoo->mapping, 0, 0, NULL, voodoo_readw, voodoo_readl, NULL, voodoo_writew, voodoo_writel, NULL,
                        MEM_MAPPING_EXTERNAL, voodoo);
        ...
        for (c = 0; c < TEX_CACHE_MAX; c++) {
                voodoo->texture_cache[0][c].data =
                        (uint32_t *)malloc((256 * 256 + 256 * 256 + 128 * 128 + 64 * 64 + 32 * 32 + 16 * 16 + 8 * 8 + 4 * 4 + 2 * 2) * 4);
                voodoo->texture_cache[0][c].base = -1; /*invalid*/
                voodoo->texture_cache[0][c].refcount = 0;
                if (voodoo->dual_tmus) {
                        voodoo->texture_cache[1][c].data = (uint32_t *)malloc(
...
static device_config_t voodoo_config[] = {
        {.name = "type",
         ...
         .selection = {{.description = "Voodoo Graphics", .value = VOODOO_1},
                       {.description = "Obsidian SB50 + Amethyst (2 TMUs)", .value = VOODOO_SB50},
                       {.description = "Voodoo 2", .value = VOODOO_2},
                       {.description = ""}},
         .default_int = 0},
        ...
        {.name = "render_threads",
         ...
         .selection = {{.description = "1", .value = 1},
                       ...
                       {.description = ""}},
         .default_int = 2},
        {.name = "sli", .description = "SLI", .type = CONFIG_BINARY, .default_int = 0},
#ifndef NO_CODEGEN
        {.name = "recompiler", .description = "Recompiler", .type = CONFIG_BINARY, .default_int = 1},
#endif
        {.type = -1}};

device_t voodoo_device = {"3DFX Voodoo Graphics", DEVICE_PCI,   voodoo_init, voodoo_close, NULL, voodoo_speed_changed, NULL,
                          voodoo_add_status_info, voodoo_config};
