...
void voodoo_recalc_tex(voodoo_t *voodoo, int tmu) {
        ...
        for (lod = 0; lod <= LOD_MAX + 2; lod++) {
                ...
                if (!(voodoo->params.tLOD[tmu] & LOD_SPLIT) || ((lod & 1) && (voodoo->params.tLOD[tmu] & LOD_ODD)) ||
...
void voodoo_use_texture(voodoo_t *voodoo, voodoo_params_t *params, int tmu) {
        ...
        do {
                for (c = 0; c < TEX_CACHE_MAX; c++) {
                        ...
                        if (voodoo->texture_cache[tmu][voodoo->texture_last_removed].refcount ==
                                    voodoo->texture_cache[tmu][voodoo->texture_last_removed].refcount_r[0] &&
                            (voodoo->render_threads == 1 ||
                             voodoo->texture_cache[tmu][voodoo->texture_last_removed].refcount ==
                                     voodoo->texture_cache[tmu][voodoo->texture_last_removed].refcount_r[1]))
                                break;
                }
                if (c == TEX_CACHE_MAX)
        ...
        lod_min = (params->tLOD[tmu] >> 2) & 15;
        lod_max = (params->tLOD[tmu] >> 8) & 15;
        //        pclog("  add new texture to %i tformat=%i %08x LOD=%i-%i tmu=%i\n", c, voodoo->params.tformat[tmu],
        //        params->texBaseAddr[tmu], lod_min, lod_max, tmu);
        ...
        for (lod = lod_min; lod <= lod_max; lod++) {
                ...
                // pclog("  LOD %i : %08x - %08x %i %i,%i\n", lod, params->tex_base[tmu][lod] & voodoo->texture_mask, addr,
                // voodoo->params.tformat[tmu], voodoo->params.tex_w_mask[tmu][lod],voodoo->params.tex_h_mask[tmu][lod]);
                ...
                case TEX_AI8:
                        for (y = 0; y < voodoo->params.tex_h_mask[tmu][lod] + 1; y++) {
                                for (x = 0; x < voodoo->params.tex_w_mask[tmu][lod] + 1; x++) {
                                        ...
                                        base[x] =
                                                makergba((dat & 0x0f) | ((dat << 4) & 0xf0), (dat & 0x0f) | ((dat << 4) & 0xf0),
                                                         (dat & 0x0f) | ((dat << 4) & 0xf0), (dat & 0xf0) | ((dat >> 4) & 0x0f));
                                }
                                tex_addr += (1 << voodoo->params.tex_shift[tmu][lod]);
                                base += (1 << shift);
                        }
                ...
                case TEX_APAL8:
                        ...
                        for (y = 0; y < voodoo->params.tex_h_mask[tmu][lod] + 1; y++) {
                                for (x = 0; x < voodoo->params.tex_w_mask[tmu][lod] + 1; x++) {
                                        ...
                                        int r = ((pal[dat].rgba.r & 3) << 6) | ((pal[dat].rgba.g & 0xf0) >> 2) |
                                                (pal[dat].rgba.r & 3);
                                        int g = ((pal[dat].rgba.g & 0xf) << 4) | ((pal[dat].rgba.b & 0xc0) >> 4) |
                                                ((pal[dat].rgba.g & 0xf) >> 2);
                ...
                case TEX_ARGB8332:
                        for (y = 0; y < voodoo->params.tex_h_mask[tmu][lod] + 1; y++) {
                                for (x = 0; x < voodoo->params.tex_w_mask[tmu][lod] + 1; x++) {
                                        uint16_t dat =
                                                *(uint16_t *)&voodoo->tex_mem[tmu][(tex_addr + x * 2) & voodoo->texture_mask];

                                        base[x] = makergba(rgb332[dat & 0xff].r, rgb332[dat & 0xff].g, rgb332[dat & 0xff].b,
                                                           dat >> 8);
                                }
                ...
                case TEX_A8Y4I2Q2:
                        ...
                        for (y = 0; y < voodoo->params.tex_h_mask[tmu][lod] + 1; y++) {
                                for (x = 0; x < voodoo->params.tex_w_mask[tmu][lod] + 1; x++) {
                                        ...
                                        base[x] = makergba(pal[dat & 0xff].rgba.r, pal[dat & 0xff].rgba.g, pal[dat & 0xff].rgba.b,
                                                           dat >> 8);
                                }
                                tex_addr += (1 << (voodoo->params.tex_shift[tmu][lod] + 1));
                                base += (1 << shift);
                        ...
                        for (d = 0; d < 4; d++) {
                                ...
                                if (addr_end != 0) {
                                        ...
                                        if (dirty_addr >= addr_start_masked && dirty_addr < addr_end_masked) {
                                                ...
                                                //                                voodoo->texture_cache[tmu][c].base);

                                                if (voodoo->texture_cache[tmu][c].refcount !=
                                                            voodoo->texture_cache[tmu][c].refcount_r[0] ||
                                                    (voodoo->render_threads == 2 &&
                                                     voodoo->texture_cache[tmu][c].refcount !=
                                                             voodoo->texture_cache[tmu][c].refcount_r[1]))
                                                        wait_for_idle = 1;

                                                voodoo->texture_cache[tmu][c].base = -1;
                                        } else {
                                                for (; addr_start <= addr_end; addr_start += (1 << TEX_DIRTY_SHIFT))
                                                        voodoo->texture_present[tmu][(addr_start & voodoo->texture_mask) >>
                                                                                     TEX_DIRTY_SHIFT] = 1;
                                        }
                                }
                        }
