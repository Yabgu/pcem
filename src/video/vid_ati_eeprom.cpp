...
enum { EEPROM_IDLE, EEPROM_WAIT, EEPROM_OPCODE, EEPROM_INPUT, EEPROM_OUTPUT };
                ...
                if (ena && !eeprom->oldena) {
                        ...
                        case EEPROM_INPUT:
                                ...
                                if (!eeprom->count) {
                                        ...
                                        switch (eeprom->opcode) {
                                        case EEPROM_OP_WRITE:
                                                //                                                pclog("EEPROM_OP_WRITE addr %02X
                                                //                                                eeprom_dat %04X\n", (eeprom->dat
                                                //                                                >> 16) & (eeprom->type ? 255 :
                                                //                                                63), eeprom->dat & 0xffff);
                                                if (!eeprom->wp) {
                                                        eeprom->data[(eeprom->dat >> 16) & (eeprom->type ? 255 : 63)] =
                                                                eeprom->dat & 0xffff;
...
int ati_eeprom_read(ati_eeprom_t *eeprom) { return eeprom->out; }
