...
static void pc1512_write(uint32_t addr, uint8_t val, void *p) {
        ...
        if ((pc1512->cgamode & 0x12) == 0x12) {
                if (pc1512->plane_write & 1)
                        pc1512->vram[addr] = val;
                if (pc1512->plane_write & 2)
                        pc1512->vram[addr | 0x4000] = val;
                if (pc1512->plane_write & 4)
                        pc1512->vram[addr | 0x8000] = val;
                if (pc1512->plane_write & 8)
        ...
        if (!pc1512->linepos) {
                ...
                if (pc1512->dispon) {
                        ...
                        for (c = 0; c < 8; c++) {
                                if ((pc1512->cgamode & 0x12) == 0x12) {
                                        ...
                                        if (pc1512->cgamode & 1)
                                                ((uint32_t *)buffer32->line[pc1512->displine])[c + (pc1512->crtc[1] << 3) + 8] =
                                                        0;
                                        else
                                                ((uint32_t *)buffer32->line[pc1512->displine])[c + (pc1512->crtc[1] << 4) + 8] =
                                                        0;
                                } else {
                                        ((uint32_t *)buffer32->line[pc1512->displine])[c] = cgapal[pc1512->cgacol & 15];
                                        if (pc1512->cgamode & 1)
                                                ((uint32_t *)buffer32->line[pc1512->displine])[c + (pc1512->crtc[1] << 3) + 8] =
                                                        cgapal[pc1512->cgacol & 15];
                                        else
                                                ((uint32_t *)buffer32->line[pc1512->displine])[c + (pc1512->crtc[1] << 4) + 8] =
                                                        cgapal[pc1512->cgacol & 15];
                                }
                        ...
                        if (pc1512->cgamode & 1) {
                                        ...
                                        if (drawcursor) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[pc1512->displine])[(x << 3) + c + 8] =
                                                                cols[(fontdat[chr + pc1512->fontbase][pc1512->sc & 7] &
                                                                      (1 << (c ^ 7)))
                                                                             ? 1
                                                                             : 0] ^
                                                                0xffffff;
                                        ...
                                        if (drawcursor) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[pc1512->displine])[(x << 4) + (c << 1) + 8] =
                                                                ((uint32_t *)buffer32
                                                                         ->line[pc1512->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                        cols[(fontdat[chr + pc1512->fontbase][pc1512->sc & 7] &
                                                                              (1 << (c ^ 7)))
                                                                                     ? 1
                                                                                     : 0] ^
                                ...
                                for (x = 0; x < 40; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                ((uint32_t *)buffer32->line[pc1512->displine])[(x << 4) + (c << 1) + 8] =
                                                        ((uint32_t *)
                                                                 buffer32->line[pc1512->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                cols[dat >> 14];
                                                dat <<= 2;
                                        }
                                }
                                ...
                                for (x = 0; x < 40; x++) {
                                        ...
                                        pc1512->ma++;
                                        for (c = 0; c < 16; c++) {
                                                ((uint32_t *)buffer32->line[pc1512->displine])[(x << 4) + c + 8] =
                        ...
                        if (pc1512->cgamode & 1)
                                hline(buffer32, 0, pc1512->displine, (pc1512->crtc[1] << 3) + 16, cols[0]);
                        else
                                hline(buffer32, 0, pc1512->displine, (pc1512->crtc[1] << 4) + 16, cols[0]);
                }
                ...
                if (pc1512->vadj) {
                        ...
                        if (pc1512->displine >= 262) // vc == (cgamode & 2) ? 111 : 27)
                        {
                                ...
                                //                                        if (vid_resize)
                                //                                        stretch_blit(vbuf,screen,0,0,xsize,(lastline-firstline)+8+1,0,0,winsizex,winsizey);
                                //                                        else
                                //                                        stretch_blit(vbuf,screen,0,0,xsize,(lastline-firstline)+8+1,0,0,xsize,((lastline-firstline)<<1)+16+2);
                                //                                        if (readflash)
                                //                                        rectfill(screen,winsizex-40,8,winsizex-8,14,0xFFFFFFFF);
                                //                                        readflash=0;
...
device_config_t pc1512_config[] = {{.name = "display_type",
                                    .description = "Display type",
                                    .type = CONFIG_SELECTION,
                                    .selection = {{.description = "PC-CM (Colour)", .value = DISPLAY_RGB},
                                                  {.description = "PC-MM (Monochrome)", .value = DISPLAY_WHITE},
                                                  {.description = ""}}},
                                   {.name = "codepage",
...
device_t pc1512_device = {"Amstrad PC1512 (video)", 0,    pc1512_init, pc1512_close, NULL,
