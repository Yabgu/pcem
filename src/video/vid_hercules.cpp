        ...
        if (!hercules->linepos) {
                ...
                if (hercules->dispon) {
                        ...
                        if ((hercules->ctrl & 2) && (hercules->ctrl2 & 1)) {
                                ...
                                for (x = 0; x < hercules->crtc[1]; x++) {
                                        dat = (hercules->vram[((hercules->ma << 1) & 0x1fff) + ca] << 8) |
                                              hercules->vram[((hercules->ma << 1) & 0x1fff) + ca + 1];
                                        hercules->ma++;
                                        for (c = 0; c < 16; c++)
                                                ((uint32_t *)buffer32->line[hercules->displine])[(x << 4) + c] =
                                                        (dat & (32768 >> c)) ? cgapal[0x7] : 0;
                                }
                        } else {
                                for (x = 0; x < hercules->crtc[1]; x++) {
                                        ...
                                        blink = ((hercules->blink & 16) && (hercules->ctrl & 0x20) && (attr & 0x80) &&
                                                 !drawcursor);
                                        if (hercules->sc == 12 && ((attr & 7) == 1)) {
                                                for (c = 0; c < 9; c++)
                                                        ((uint32_t *)buffer32->line[hercules->displine])[(x * 9) + c] =
                                                                mdacols[attr][blink][1];
                                        } else {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[hercules->displine])[(x * 9) + c] =
                                                                mdacols[attr][blink]
                                                                       [(fontdatm[chr][hercules->sc] & (1 << (c ^ 7))) ? 1 : 0];
                                                if ((chr & ~0x1f) == 0xc0)
                                                        ((uint32_t *)buffer32->line[hercules->displine])[(x * 9) + 8] =
                                                                mdacols[attr][blink][fontdatm[chr][hercules->sc] & 1];
                                                else
                                                        ((uint32_t *)buffer32->line[hercules->displine])[(x * 9) + 8] =
                                                                mdacols[attr][blink][0];
                                        }
                                        hercules->ma++;
                                        if (drawcursor) {
                                                for (c = 0; c < 9; c++)
                ...
                if (hercules->vadj) {
                        ...
                        if (oldvc == hercules->crtc[4]) {
                                ...
                                if (!hercules->vadj)
                                        hercules->ma = hercules->maback =
                                                (hercules->crtc[13] | (hercules->crtc[12] << 8)) & 0x3fff;
...
static device_config_t hercules_config[] = {{.name = "display_type",
                                             .description = "Display type",
                                             .type = CONFIG_SELECTION,
                                             .selection = {{.description = "Green", .value = DISPLAY_GREEN},
...
device_t hercules_device = {"Hercules", 0,    hercules_init,  hercules_close, NULL, hercules_speed_changed,
                            NULL,       NULL, hercules_config};
