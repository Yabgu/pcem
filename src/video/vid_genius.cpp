...
void genius_textline(genius_t *genius, uint8_t background) {
        ...
        if ((genius->mda_crtc[10] & 0x60) == 0x20) {
                ...
                cursorline = ((genius->mda_crtc[10] & 0x1F) <= sc) && ((genius->mda_crtc[11] & 0x1F) >= sc);
        }

        for (x = 0; x < w; x++) {
                chr = framebuf[(addr + 2 * x) & 0x3FFF];
                attr = framebuf[(addr + 2 * x + 1) & 0x3FFF];
                drawcursor = ((ma == ca) && cursorline && genius->enabled && (genius->mda_ctrl & 8));
                ...
                blink = ((genius->blink & 16) && (genius->mda_ctrl & 0x20) && (attr & 0x80) && !drawcursor);

                if (genius->mda_ctrl & 0x20)
                        attr &= 0x7F;
                /* MDA underline */
...
void *genius_init() {
        ...
        for (c = 0; c < 256; c++) {
                ...
                if (c & 8)
                        mdacols[c][0][1] = genius_pal[3];
                else
                        mdacols[c][0][1] = genius_pal[2];
        }
...
static int genius_available() { return rom_present("8x12.bin"); }
...
device_t genius_device = {"Genius VHR", 0, genius_init, genius_close, genius_available, genius_speed_changed, NULL, NULL};
