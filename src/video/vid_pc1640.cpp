...
void pc1640_out(uint16_t addr, uint8_t val, void *p) {
        ...
        if (pc1640->cga_enabled)
                cga_out(addr, val, &pc1640->cga);
        else
                ega_out(addr, val, &pc1640->ega);
}
...
uint8_t pc1640_in(uint16_t addr, void *p) {
        ...
        if (pc1640->cga_enabled)
                return cga_in(addr, &pc1640->cga);
        else
                return ega_in(addr, &pc1640->ega);
}
...
device_t pc1640_device = {"Amstrad PC1640 (video)", 0, pc1640_init, pc1640_close, NULL, pc1640_speed_changed, NULL, NULL};
