...
static uint8_t crtcmask[32] = {0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0xff, 0xff, 0xf3, 0x1f, 0x7f, 0x1f, 0x3f, 0xff, 0x3f, 0xff,
...
static void tandysl_write(uint32_t addr, uint8_t val, void *p) {
        ...
        egawrites++;
        //        pclog("Tandy VRAM write %05X %02X %04X:%04X  %02x %x\n",addr,val,CS,pc,tandy->array[5],
        //        (uintptr_t)&tandy->b8000[addr & 0xffff] - (uintptr_t)ram);
        if (tandy->array[5] & 1)
...
static void tandysl_poll(void *p) {
        ...
        if (!tandy->linepos) {
                ...
                if (tandy->dispon) {
                        ...
                        for (c = 0; c < 8; c++) {
                                if (tandy->array[3] & 4) {
                                        ...
                                        if (tandy->mode & 1)
                                                ((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 3) + 8] =
                                                        cols[0];
                                        else
                                                ((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 4) + 8] =
                                                        cols[0];
                        ...
                        if (tandy->array[5] & 1) /*640x200x16*/
                        {
                                for (x = 0; x < tandy->crtc[1] * 2; x++) {
                                        ...
                                        tandy->ma++;
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 2) + 8] =
                                                cgapal[tandy->array[((dat >> 12) & 0xf) /*tandy->array[1])*/ + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 2) + 9] =
                                                cgapal[tandy->array[((dat >> 8) & 0xf) /*tandy->array[1])*/ + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 2) + 10] =
                                                cgapal[tandy->array[((dat >> 4) & 0xf) /*tandy->array[1])*/ + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 2) + 11] =
                                                cgapal[tandy->array[(dat & 0xf) /*tandy->array[1])*/ + 16]];
                                }
                        } else if ((tandy->array[3] & 0x10) && (tandy->mode & 1)) /*320x200x16*/
                        {
                                for (x = 0; x < tandy->crtc[1]; x++) {
                                        ...
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 8] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 9] =
                                                        cgapal[tandy->array[((dat >> 12) & tandy->array[1]) + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 10] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 11] =
                                                        cgapal[tandy->array[((dat >> 8) & tandy->array[1]) + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 12] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 13] =
                                                        cgapal[tandy->array[((dat >> 4) & tandy->array[1]) + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 14] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 15] =
                                                        cgapal[tandy->array[(dat & tandy->array[1]) + 16]];
                                }
                                        ...
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 8] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 9] =
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 10] =
                                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 11] =
                                                                        cgapal[tandy->array[((dat >> 12) & tandy->array[1]) +
                                                                                            16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 12] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 13] =
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 14] =
                                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 15] =
                                                                        cgapal[tandy->array[((dat >> 8) & tandy->array[1]) + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 16] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 17] =
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 18] =
                                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 19] =
                                                                        cgapal[tandy->array[((dat >> 4) & tandy->array[1]) + 16]];
                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 20] =
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 21] =
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 22] =
                                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + 23] =
                                                                        cgapal[tandy->array[(dat & tandy->array[1]) + 16]];
                                }
                        } else if (tandy->array[3] & 0x08) /*640x200x4 - this implementation is a complete guess!*/
                        {
                                for (x = 0; x < tandy->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                chr = (dat >> 7) & 1;
                                                chr |= ((dat >> 14) & 2);
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 8 + c] =
                                                        cgapal[tandy->array[(chr & tandy->array[1]) + 16]];
                                                dat <<= 1;
                                        }
                                }
                                ...
                                for (x = 0; x < tandy->crtc[1]; x++) {
                                        ...
                                        if (tandy->sc & 8) {
                                                ...
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + c + 8] =
                                                                cols[(fontdat[chr][tandy->sc & 7] & (1 << (c ^ 7))) ? 1 : 0];
                                        }
                                        //                                        if (!((ma^(crtc[15]|(crtc[14]<<8)))&0x3FFF))
                                        //                                        printf("Cursor match! %04X\n",ma);
                                        if (drawcursor) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + c + 8] ^=
                                                                0xffffff;
                                        ...
                                        if (tandy->sc & 8) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + (c << 1) + 8] =
                                                                ((uint32_t *)buffer32
                                                                         ->line[tandy->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                        cols[0];
                                        } else {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + (c << 1) + 8] =
                                                                ((uint32_t *)buffer32
                                                                         ->line[tandy->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                        cols[(fontdat[chr][tandy->sc & 7] & (1 << (c ^ 7))) ? 1
                                                                                                                            : 0];
                                        }
                                        if (drawcursor) {
                                                for (c = 0; c < 16; c++)
                                                        ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + c + 8] ^=
                                                                0xffffff;
                                        }
                                }
                                ...
                                for (x = 0; x < tandy->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 4) + (c << 1) + 8] =
                                                        ((uint32_t *)
                                                                 buffer32->line[tandy->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                cols[dat >> 14];
                                                dat <<= 2;
                                        }
                                }
                        ...
                        if (tandy->array[3] & 4) {
                                if (tandy->mode & 1)
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 3) + 16,
                                              cgapal[tandy->array[2] & 0xf]);
                                else
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 4) + 16,
                                              cgapal[tandy->array[2] & 0xf]);
                        } else {
                                cols[0] = cgapal[((tandy->mode & 0x12) == 0x12) ? 0 : (tandy->col & 0xf)];
                                if (tandy->mode & 1)
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 3) + 16, cols[0]);
                                else
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 4) + 16, cols[0]);
                        }
                }
                if (tandy->mode & 1)
                        x = (tandy->crtc[1] << 3) + 16;
                ...
                if (tandy->sc == (tandy->crtc[11] & 31) ||
                    ((tandy->crtc[8] & 3) == 3 && tandy->sc == ((tandy->crtc[11] & 31) >> 1))) {
                        tandy->con = 0;
                        tandy->coff = 1;
                }
                if (tandy->vadj) {
                        ...
                        if (oldvc == tandy->crtc[4]) {
                                ...
                                if ((tandy->crtc[10] & 0x60) == 0x20)
                                        tandy->cursoron = 0;
                                else
                                        tandy->cursoron = tandy->blink & 16;
                                //                                printf("CRTC10 %02X %i\n",crtc[10],cursoron);
                        }
                        if (tandy->vc == tandy->crtc[7]) {
                                ...
                                //                                tandy->stat|=8;
                                if (tandy->crtc[7]) {
                                        //                                        printf("Lastline %i Firstline %i  %i   %i
                                        ...
                                        video_blit_memtoscreen(0, tandy->firstline - 4, 0,
                                                               (tandy->lastline - tandy->firstline) + 8, xsize,
                                                               (tandy->lastline - tandy->firstline) + 8);
                                        ...
                                        } else if (tandy->array[3] & 0x10) /*160x200x16*/
                                        {
                                                video_res_x /= 4;
                                                video_bpp = 4;
                                        } else if (tandy->array[3] &
                                                   0x08) /*640x200x4 - this implementation is a complete guess!*/
                                                video_bpp = 2;
                                        else if (tandy->mode & 1) {
                                                video_res_x /= 8;
                ...
                if ((tandy->sc == (tandy->crtc[10] & 31) ||
                     ((tandy->crtc[8] & 3) == 3 && tandy->sc == ((tandy->crtc[10] & 31) >> 1))))
                        tandy->con = 1;
        }
}

static void *tandysl_init() {
        tandysl_t *tandy = (tandysl_t *)malloc(sizeof(tandysl_t));
        ...
        mem_mapping_add(&tandy->ram_mapping, 0x80000, 0x20000, tandysl_ram_read, NULL, NULL, tandysl_ram_write, NULL, NULL, NULL,
                        0, tandy);
...
device_t tandysl_device = {"Tandy 1000SL (video)", 0, tandysl_init, tandysl_close, NULL, tandysl_speed_changed, NULL, NULL};
