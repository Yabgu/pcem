...
enum { S3_VISION864, S3_TRIO32, S3_TRIO64 };

enum { VRAM_4MB = 0, VRAM_8MB = 3, VRAM_2MB = 4, VRAM_1MB = 6, VRAM_512KB = 7 };
...
static void s3_accel_out_fifo(s3_t *s3, uint16_t port, uint8_t val) {
        ...
        case 0xe2eb:
                ...
                if ((s3->accel.multifunc[0xa] & 0xc0) == 0x80 && (s3->accel.cmd & 0x600) == 0x600 && (s3->accel.cmd & 0x100) &&
                    s3->chip == S3_TRIO32) {
                        ...
                        s3_accel_start(8, 1, s3->accel.pix_trans[0], 0, s3);
                           ...
                           (s3->accel.cmd & 0x100))
                        s3_accel_start(32, 1,
                                       s3->accel.pix_trans[0] | (s3->accel.pix_trans[1] << 8) | (s3->accel.pix_trans[2] << 16) |
...
#define READ_SRC(addr, dat)                                                                                                      \
        if (s3->bpp == 0)                                                                                                        \
                dat = svga->vram[dword_remap(addr) & s3->vram_mask];                                                             \
        else if (s3->bpp == 1)                                                                                                   \
                dat = vram_w[dword_remap_w(addr) & (s3->vram_mask >> 1)];                                                        \
        else                                                                                                                     \
                dat = vram_l[dword_remap_l(addr) & (s3->vram_mask >> 2)];                                                        \
        if (vram_mask)                                                                                                           \
                dat = ((dat & rd_mask) == rd_mask);

#define READ_DST(addr, dat)                                                                                                      \
        if (s3->bpp == 0)                                                                                                        \
                dat = svga->vram[dword_remap(addr) & s3->vram_mask];                                                             \
        else if (s3->bpp == 1)                                                                                                   \
                dat = vram_w[dword_remap_w(addr) & (s3->vram_mask >> 1)];                                                        \
...
void s3_accel_start(int count, int cpu_input, uint32_t mix_dat, uint32_t cpu_dat, s3_t *s3) {
        ...
        case 0x400:
                ...
                break;
        case 0x600:
                mix_mask = (s3->chip == S3_TRIO32) ? 0x80 : 0x80000000;
                break;
        }
        ...
        case 6:                 /*BitBlt*/
                ...
                if (!cpu_input && frgd_mix == 3 && !vram_mask && !compare_mode && (s3->accel.cmd & 0xa0) == 0xa0 &&
                                ...
                                if ((s3->accel.poly_x & 0xfff) >= clip_l && (s3->accel.poly_x & 0xfff) <= clip_r &&
                                    (s3->accel.poly_cy & 0xfff) >= clip_t && (s3->accel.poly_cy & 0xfff) <= clip_b) {
                                        ...
                                        case 2:
                                                ...
                                                break;
                                        case 3:
                                                READ_SRC(s3->accel.src + pat_x, src_dat);
                                                break;
                                        }
...
uint8_t s3_pci_read(int func, int addr, void *p) {
        ...
        case 0x30:
                return s3->pci_regs[0x30] & 0x01; /*BIOS ROM address*/
        case 0x31:
                return 0x00;
        case 0x32:
...
static void *s3_init(char *bios_fn, int chip) {
        ...
        mem_mapping_add(&s3->linear_mapping, 0, 0, svga_read_linear, svga_readw_linear, svga_readl_linear, svga_write_linear,
                        svga_writew_linear, svga_writel_linear, NULL, MEM_MAPPING_EXTERNAL, &s3->svga);
        mem_mapping_add(&s3->mmio_mapping, 0xa0000, 0x10000, s3_accel_read, NULL, NULL, s3_accel_write, s3_accel_write_w,
                        s3_accel_write_l, NULL, MEM_MAPPING_EXTERNAL, s3);
        mem_mapping_disable(&s3->mmio_mapping);
...
int s3_phoenix_trio64_available() { return rom_present("86c764x1.bin"); }
...
void s3_add_status_info(char *s, int max_len, void *p) {
        ...
        sprintf(temps, "%f%% CPU\n%f%% CPU (real)\n\n", ((double)s3->blitter_time * 100.0) / timer_freq,
                ((double)s3->blitter_time * 100.0) / status_diff);
        strncat(s, temps, max_len);

        s3->blitter_time = 0;
}

static device_config_t s3_bahamas64_config[] = {{.name = "memory",
...
static device_config_t s3_9fx_config[] = {{.name = "memory",
                                           .description = "Memory size",
                                           .type = CONFIG_SELECTION,
                                           .selection = {{.description = "1 MB", .value = 1},
                                                         {.description = "2 MB", .value = 2},
...
static device_config_t s3_phoenix_trio32_config[] = {{.name = "memory",
                                                      .description = "Memory size",
                                                      .type = CONFIG_SELECTION,
                                                      .selection = {{.description = "512 KB", .value = 0},
...
static device_config_t s3_phoenix_trio64_config[] = {{.name = "memory",
                                                      .description = "Memory size",
                                                      .type = CONFIG_SELECTION,
                                                      .selection = {{.description = "1 MB", .value = 1},
                                                                    {.description = "2 MB", .value = 2},
...
device_t s3_bahamas64_device = {"Paradise Bahamas 64 (S3 Vision864)",
                                0,
                                s3_bahamas64_init,
                                s3_close,
...
device_t s3_9fx_device = {"Number 9 9FX (S3 Trio64)",
                          0,
                          s3_9fx_init,
                          s3_close,
                          s3_9fx_available,
...
device_t s3_phoenix_trio32_device = {"Phoenix S3 Trio32",
                                     0,
...
device_t s3_phoenix_trio64_device = {"Phoenix S3 Trio64",
                                     0,
