...
void pcjr_poll(void *p) {
        ...
        if (!pcjr->linepos) {
                ...
                if (pcjr->dispon) {
                        ...
                        case 0x13: /*320x200x16*/
                                for (x = 0; x < pcjr->crtc[1]; x++) {
                                        ...
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 8] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 9] =
                                                        pcjr->array[((dat >> 12) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 10] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 11] =
                                                        pcjr->array[((dat >> 8) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 12] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 13] =
                                                        pcjr->array[((dat >> 4) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 14] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 15] =
                                                        pcjr->array[(dat & pcjr->array[1]) + 16];
                                }
                                break;
                                        ...
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 8] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 9] =
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 10] =
                                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 11] =
                                                                        pcjr->array[((dat >> 12) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 12] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 13] =
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 14] =
                                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 15] =
                                                                        pcjr->array[((dat >> 8) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 16] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 17] =
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 18] =
                                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 19] =
                                                                        pcjr->array[((dat >> 4) & pcjr->array[1]) + 16];
                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 20] =
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 21] =
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 22] =
                                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + 23] =
                                                                        pcjr->array[(dat & pcjr->array[1]) + 16];
                                }
                                break;
                        case 0x03: /*640x200x4*/
                                for (x = 0; x < pcjr->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                chr = (dat >> 7) & 1;
                                                chr |= ((dat >> 14) & 2);
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + 8 + c] =
                                                        pcjr->array[(chr & pcjr->array[1]) + 16];
                                                dat <<= 1;
                                        }
                                }
                                break;
                        case 0x01: /*80 column text*/
                                for (x = 0; x < pcjr->crtc[1]; x++) {
                                        ...
                                        if (pcjr->sc & 8) {
                                                ...
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 3) + c + 8] =
                                        ...
                                        if (pcjr->sc & 8) {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + (c << 1) + 8] =
                                                                ((uint32_t *)buffer32
                                                                         ->line[pcjr->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                        cols[0];
                                        } else {
                                                for (c = 0; c < 8; c++)
                                                        ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + (c << 1) + 8] =
                                                                ((uint32_t *)buffer32
                                                                         ->line[pcjr->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                        cols[(fontdat[chr][pcjr->sc & 7] & (1 << (c ^ 7))) ? 1
                                                                                                                           : 0];
                                        }
                                        if (drawcursor) {
                                                for (c = 0; c < 16; c++)
                        ...
                        case 0x02: /*320x200x4*/
                                ...
                                for (x = 0; x < pcjr->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                ((uint32_t *)buffer32->line[pcjr->displine])[(x << 4) + (c << 1) + 8] =
                                                        ((uint32_t *)
                                                                 buffer32->line[pcjr->displine])[(x << 4) + (c << 1) + 1 + 8] =
                                                                cols[dat >> 14];
                                                dat <<= 2;
                                        }
                                }
                        ...
                        if (pcjr->array[3] & 4) {
                                if (pcjr->array[0] & 1)
                                        hline(buffer32, 0, pcjr->displine, (pcjr->crtc[1] << 3) + 16, pcjr->array[2] & 0xf);
                                else
                                        hline(buffer32, 0, pcjr->displine, (pcjr->crtc[1] << 4) + 16, pcjr->array[2] & 0xf);
                        } else {
                                //                                cols[0] = ((pcjr->mode & 0x12) == 0x12) ? 0 : (pcjr->col & 0xf)
                                //                                + 16;
                                cols[0] = pcjr->array[0 + 16];
                                if (pcjr->array[0] & 1)
                                        hline(buffer32, 0, pcjr->displine, (pcjr->crtc[1] << 3) + 16, cols[0]);
                                else
                                        hline(buffer32, 0, pcjr->displine, (pcjr->crtc[1] << 4) + 16, cols[0]);
                        }
                }
                if (pcjr->array[0] & 1)
                        x = (pcjr->crtc[1] << 3) + 16;
                ...
                if (pcjr->composite) {
                        ...
                        for (c = 0; c < x; c++)
                                ((uint32_t *)buffer32->line[pcjr->displine])[c] =
                                        cgapal[((uint32_t *)buffer32->line[pcjr->displine])[c] & 0xf];
                }
                pcjr->sc = oldsc;
                ...
                if (pcjr->vadj) {
                        ...
                        if (pcjr->vc == pcjr->crtc[7]) {
                                ...
                                if (pcjr->crtc[7]) {
                                        ...
                                        video_blit_memtoscreen(0, pcjr->firstline - 4, 0, (pcjr->lastline - pcjr->firstline) + 8,
                                                               xsize, (pcjr->lastline - pcjr->firstline) + 8);
...
device_t pcjr_video_device = {"IBM PCjr (video)", 0, pcjr_video_init, pcjr_video_close, NULL, pcjr_speed_changed, NULL, NULL};

static device_config_t pcjr_config[] = {{.name = "display_type",
                                         .description = "Display type",
                                         .type = CONFIG_SELECTION,
                                         .selection = {{.description = "RGB", .value = PCJR_RGB},
...
/*This isn't really a device as such - more of a convenient way to hook in the
  config information*/
device_t pcjr_device = {"IBM PCjr", 0, NULL, NULL, NULL, NULL, NULL, NULL, pcjr_config};
