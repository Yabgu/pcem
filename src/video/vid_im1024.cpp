...
int im1024_input_byte(pgc_core_t *pgc, uint8_t *result) {
        ...
        while ((im1024->fifo_wrptr == im1024->fifo_rdptr) && (pgc->mapram[0x300] == pgc->mapram[0x301])) {
...
void hndl_pline(pgc_core_t *pgc) {
        ...
        for (n = 1; n < count; n++) {
                linemask = pgc_draw_line(pgc, x[n - 1] << 16, y[n - 1] << 16, x[n] << 16, y[n] << 16, linemask);
        }
}
...
void blkmov_row(pgc_core_t *pgc, int16_t x0, int16_t x1, int16_t x2, int16_t sy, int16_t ty) {
        int16_t x;
        uint8_t src[1024];
        uint8_t dst[1024];
...
static void hndl_twrite(pgc_core_t *pgc) {
        ...
        for (n = 0; n < count; n++) {
                ...
                PGCLOG(("ch=0x%02x w=%d h=%d wb=%d\n", buf[n], im1024->fontx[buf[n]], im1024->fonty[buf[n]], wb));
 ...
 *       does not use them.
 */
static const pgc_command_t im1024_commands[] = {{"BLKMOV", 0xDF, hndl_blkmov, pgc_parse_words, 6},
...
void *im1024_init() {
        ...
        mem_mapping_set_handler(&im1024->pgc.mapping, im1024_read, NULL, NULL, im1024_write, NULL, NULL);
...
device_t im1024_device = {"Image Manager 1024", 0, im1024_init, im1024_close, NULL, im1024_speed_changed, NULL, NULL, NULL};
