...
void tandy_poll(void *p) {
        ...
        if (!tandy->linepos) {
                ...
                if (tandy->dispon) {
                        ...
                        for (c = 0; c < 8; c++) {
                                if (tandy->array[3] & 4) {
                                        ...
                                        if (tandy->mode & 1)
                                                ((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 3) + 8] = 0;
                                        else
                                                ((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 4) + 8] = 0;
                                } else {
                                ...
                                for (x = 0; x < tandy->crtc[1]; x++) {
                                        ...
                                        for (c = 0; c < 8; c++) {
                                                ...
                                                ((uint32_t *)buffer32->line[tandy->displine])[(x << 3) + 8 + c] =
                                                        tandy->array[(chr & tandy->array[1]) + 16];
                                                dat <<= 1;
                                        }
                                }
                        ...
                        if (tandy->array[3] & 4) {
                                if (tandy->mode & 1)
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 3) + 16, tandy->array[2] & 0xf);
                                else
                                        hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 4) + 16, tandy->array[2] & 0xf);
                        } else {
                ...
                if (tandy->composite) {
                        ...
                        for (c = 0; c < x; c++)
                                ((uint32_t *)buffer32->line[tandy->displine])[c] =
                                        cgapal[((uint32_t *)buffer32->line[tandy->displine])[c] & 0xf];
                }
...
void *tandy_init() {
        ...
        mem_mapping_add(&tandy->ram_mapping, 0x80000, 0x20000, tandy_ram_read, NULL, NULL, tandy_ram_write, NULL, NULL, NULL, 0,
                        tandy);
...
device_t tandy_device = {"Tandy 1000 (video)", 0, tandy_init, tandy_close, NULL, tandy_speed_changed, NULL, NULL};

static device_config_t tandy_config[] = {{.name = "display_type",
                                          .description = "Display type",
                                          .type = CONFIG_SELECTION,
                                          .selection = {{.description = "RGB", .value = TANDY_RGB},
...
/*These aren't really devices as such - more of a convenient way to hook in the
  config information*/
device_t tandy1000_device = {"Tandy 1000", 0, NULL, NULL, NULL, NULL, NULL, NULL, tandy_config};
device_t tandy1000hx_device = {"Tandy 1000HX", 0, NULL, NULL, NULL, NULL, NULL, NULL, tandy_config};
