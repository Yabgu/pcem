...
void m24_poll(void *p) {
        ...
        int oldsc;
        if (!m24->linepos) {
                //                pclog("Line poll  %i %i %i %i - %04X %i %i %i\n", m24_lineff, vc, sc, vadj, ma, firstline,
                //                lastline, displine);
                ...
                if (m24->dispon) {
                        ...
                        for (c = 0; c < 8; c++) {
                                if ((m24->cgamode & 0x12) == 0x12) {
                                        ...
                                        if (m24->cgamode & 1)
                                                ((uint32_t *)buffer32->line[m24->displine])[c + (m24->crtc[1] << 3) + 8] =
                                                        cgapal[0];
                                        else
                                                ((uint32_t *)buffer32->line[m24->displine])[c + (m24->crtc[1] << 4) + 8] =
                                                        cgapal[0];
                                } else {
                                        ((uint32_t *)buffer32->line[m24->displine])[c] = cgapal[m24->cgacol & 15];
                                        if (m24->cgamode & 1)
                                                ((uint32_t *)buffer32->line[m24->displine])[c + (m24->crtc[1] << 3) + 8] =
                                                        cgapal[m24->cgacol & 15];
                                        else
                                                ((uint32_t *)buffer32->line[m24->displine])[c + (m24->crtc[1] << 4) + 8] =
                                                        cgapal[m24->cgacol & 15];
                                }
                        ...
                        if (m24->cgamode & 1) {
                                ...
                                for (x = 0; x < m24->crtc[1]; x++) {
                                        dat = (m24->vram[((m24->ma << 1) & 0x1fff) + dat2] << 8) |
                        ...
                        if (m24->cgamode & 1)
                                hline(buffer32, 0, m24->displine, (m24->crtc[1] << 3) + 16, cols[0]);
                        else
                                hline(buffer32, 0, m24->displine, (m24->crtc[1] << 4) + 16, cols[0]);
                }
