...
static inline int codegen_texture_fetch(uint8_t *code_block, voodoo_t *voodoo, voodoo_params_t *params, voodoo_state_t *state,
                                        int block_pos, int tmu) {
        if (params->textureMode[tmu] & 1) {
                ...
                addbyte(0x9f);
                addlong(offsetof(voodoo_state_t, lod_frac[0]) + (tmu) * sizeof(((voodoo_state_t *)0)->lod_frac[0]));
        ...
        if (params->fbzColorPath & FBZCP_TEXTURE_ENABLED) {
                if (voodoo->bilinear_enabled && (params->textureMode[tmu] & 6)) {
                        ...
                        addbyte(0x8f);
                        addlong(offsetof(voodoo_state_t, tex_lod[0]) + (tmu) * sizeof(((voodoo_state_t *)0)->tex_lod[0]));
...
static inline void voodoo_generate(uint8_t *code_block, voodoo_t *voodoo, voodoo_params_t *params, voodoo_state_t *state,
                                   int depthop) {
        int block_pos = 0;
        int z_skip_pos = 0;
        int a_skip_pos = 0;
        ...
        if ((params->alphaMode & ((1 << 0) | (1 << 4))) ||
            (!(cc_mselect == 0 && cc_reverse_blend == 0) &&
             (cc_mselect == CC_MSELECT_AOTHER || cc_mselect == CC_MSELECT_ALOCAL))) {
                /*EBX = a_other*/
                switch (a_sel) {
                case A_SEL_ITER_A:
                ...
                if (params->fogMode & FOG_CONSTANT) {
                        ...
                        case 0:
                                ...
                                /*                                int fog_idx = (w_depth >> 10) & 0x3f;
                                                                ...
                                                                fog_a += (params->fogTable[fog_idx].dfog * ((w_depth >> 2) &
                                   0xff)) >> 10;*/
                                break;
...
static inline void *voodoo_get_block(voodoo_t *voodoo, voodoo_params_t *params, voodoo_state_t *state, int odd_even) {
        ...
        for (c = 0; c < 8; c++) {
                ...
                if (state->xdir == data->xdir && params->alphaMode == data->alphaMode && params->fbzMode == data->fbzMode &&
                    params->fogMode == data->fogMode && params->fbzColorPath == data->fbzColorPath &&
                    (voodoo->trexInit1[0] & (1 << 18)) == data->trexInit1 && params->textureMode[0] == data->textureMode[0] &&
                    params->textureMode[1] == data->textureMode[1] && (params->tLOD[0] & LOD_MASK) == data->tLOD[0] &&
...
void voodoo_codegen_init(voodoo_t *voodoo) {
...
#else
        voodoo->codegen_data = mmap(0, sizeof(voodoo_x86_data_t) * BLOCK_NUM * 4, PROT_READ | PROT_WRITE | PROT_EXEC,
        ...
        for (c = 0; c < 256; c++) {
                ...
                bilinear_lookup[c * 2] =
                        _mm_set_epi32(d[1] | (d[1] << 16), d[1] | (d[1] << 16), d[0] | (d[0] << 16), d[0] | (d[0] << 16));
                bilinear_lookup[c * 2 + 1] =
                        _mm_set_epi32(d[3] | (d[3] << 16), d[3] | (d[3] << 16), d[2] | (d[2] << 16), d[2] | (d[2] << 16));
        }
